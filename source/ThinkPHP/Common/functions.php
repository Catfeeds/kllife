...
*/
function get_ip_location($ip, $type=0) {
	if ($type == 0) { 	// 高德查询接口		
		$url = "http://restapi.amap.com/v3/ip?ip=".$ip."&output=JSON&key=62c6054a41751763f232d3cfb867f98e";
	} else if ($type == 1) {	 // 淘宝查询接口
		$url = "http://ip.taobao.com/service/getIpInfo.php?ip=".$ip; 
	} else {
		error(-1, '查询类型错误');
	}
	
	$ipds = file_get_contents($url); //调用淘宝接口获取信息
	$ip = json_decode($ipds, true);
	if ($type == 0) { 	// 高德查询接口
		$ipData['province'] = $ip['province'];
		$ipData['city'] = $ip['city'];
		return $ipData;
	} else if ($type == 1) {	 // 淘宝查询接口
		$ipData['province'] = $ip['data']['province'];
		$ipData['city'] = $ip['data']['city'];	
		return $ipData;
	} else {
		error(-1, '查询类型错误');
	}
