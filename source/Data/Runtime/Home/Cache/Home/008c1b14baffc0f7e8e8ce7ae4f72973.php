...
<div class="lunbo">
	...
	<div class="lubo">
		<ul class="lubo_box">
			<li style=";display: block;"><a href="<?php echo ($lunbo["lunbo1_url"]); ?>" style="background-image:url(<?php echo ($lunbo["lunbo1"]); ?>); background-size: 100% 100%;"></a></li>
			<li><a href="<?php echo ($lunbo["lunbo2_url"]); ?>" style="background-image:url(<?php echo ($lunbo["lunbo2"]); ?>); background-size: 100% 100%;"></a></li>
			<li><a href="<?php echo ($lunbo["lunbo3_url"]); ?>" style="background-image:url(<?php echo ($lunbo["lunbo3"]); ?>); background-size: 100% 100%;"></a></li>
		</ul>
	</div>
</div>
		...
		<div class="special main">
			...
			<div class="special-exhibition">
				...
				<ul>
					...
					<?php $__FOR_START_1782772197__=0;$__FOR_END_1782772197__=12;for($k=$__FOR_START_1782772197__;$k < $__FOR_END_1782772197__;$k+=1){ $zt = $set_subjects['zhuanti'.$k]; if (empty($zt['url'])) { continue; } ?>
...
<style type="text/css">
	...
	<div id="content" style="margin-top: 0;">
		...
		<!-- 列表菜单 -->
		<div class="list-menu" style="margin-top: 0;">
			...
			<?php if(empty($lines) == true): ?><span class="no-more-data" style="text-indent:2em;">暂时没有分销的产品，敬请期待站长发布...</span><?php endif; ?>
			
			<!-- 单排列表 -->
			<div class="single-row">
				<?php if(is_array($lines)): $i = 0; $__LIST__ = $lines;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$line): $mod = ($i % 2 );++$i;?><div class="single-row-content" data-bind-id="<?php echo ($line["bind_id"]); ?>" data-id="<?php echo ($line["id"]); ?>">
						<a href="<?php echo U("line/info");?>/id/<?php echo ($line["id"]); ?>/duid/<?php echo ($line["bind_user_id"]); ?>" target="_blank"></a>
						<div class="single-row-img">
						...
						<div class="single-row-describe">
							<div class="single-row-describe-top">
								...
								<div class="single-row-describe-price">
									<h6>
										<?php if($line["free_line"] == '0' and (empty($line['min_batch']) == true or empty($line['min_batch']['price_state']) == false)): ?><s class="price">核算中</s>
												<strong class="cut_price">核算中</strong>
										<?php else: ?>
											<!--<s class="price"><?php echo ($line["min_batch"]["price_adult"]); ?>元</s>-->
											<strong class="cut_price"><?php echo ($line["min_batch"]["adult_cut"]); ?></strong>元起<?php endif; ?>
									</h6>
								</div>
							</div>
							<div class="single-row-describe-bottom">
								<span class="travel_date">旅行时间： <?php echo ($line["start_time"]); ?>&nbsp;至&nbsp;<?php echo ($line["end_time"]); ?></span>
								<em class="appointment" href="<?php echo U("line/info");?>/id/<?php echo ($line["id"]); ?>/duid/<?php echo ($line["bind_user_id"]); ?>"></em>
							</div>
						</div>
...
//			console.log(JSON.stringify(jsonData));
			...
			$.post('<?php echo U("fenxiao/welcome");?>', jsonData, function(data){
				if (data.result.errno == 0) {
					...
					if (data.ds != null && data.ds != 'undefined') {
						...
						for (var i=0; i < data.ds.length; i++) {
							...
							$(lineObj).find('a').attr('href','<?php echo U("line/info");?>/id/'+ d.id+'/duid/'+d.bind_user_id);
							...
							if (d.min_batch != null) {
								if (d.min_batch.price_state != null) {
									$(lineObj).find('.price').parent().html('<s class="price">核算中</s><strong class="cut_price">核算中</strong>');								
								} else {
									$(lineObj).find('.price').html(d.min_batch.price_adult+'元');
									$(lineObj).find('.cut_price').html(d.min_batch.adult_cut+'元起');
								}
							} else {				
								if (d.free_line == 0) {
									$(lineObj).find('.price').html('0元');
									$(lineObj).find('.cut_price').html('0');
								} else {
									$(lineObj).find('.price').parent().html('<s class="price">核算中</s><strong class="cut_price">核算中</strong>');								
								}					   	
							}
							...
							$(mLineObj).find('a').attr('href','<?php echo U("line/info");?>/id/'+ d.id+'/duid/'+d.bind_user_id);
							...
							if (d.min_batch != null) {
								if (d.min_batch.price_state != null) {
									$(mLineObj).find('.price').parent().html('<s class="price">核算中</s><strong class="cut_price">核算中</strong>');								
								} else {
									$(mLineObj).find('.price').html(d.min_batch.price_adult+'元');
									$(mLineObj).find('.cut_price').html(d.min_batch.adult_cut);
								}
							} else {		
								if (d.free_line == 0) {
									$(mLineObj).find('.price').html('0元');
									$(mLineObj).find('.cut_price').html('0');
								} else {
									$(mLineObj).find('.price').parent().html('<s class="price">核算中</s><strong class="cut_price">核算中</strong>');								
								}   	
							}
