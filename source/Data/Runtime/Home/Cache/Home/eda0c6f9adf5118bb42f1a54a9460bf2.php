...
<link rel="stylesheet" href="/source/Static/qinglvpai/css/iconfont.css">
<link rel="stylesheet" href="/source/Static/qinglvpai/css/welcome.css">
<style>
...
<!--header-->
<div class="header">
    <div class="banner" style="background-image: url(<?php echo ($qlp_set["content_banner"]); ?>);"></div>
</div>
<!--什么是跟拍游？-->
...
<div class="list-group">
    <div class="line">
        ...
        <!--<div class="name">
            ...
            <div>
                <ul>
                	<li>全部</li>
                	<?php if(is_array($qlp_set)): $i = 0; $__LIST__ = $qlp_set;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i; if(stripos($key, 'travel_tab_') === 0 and stripos($key, '_line_') === false and stripos($key, '_line_image') === false): ?><li><?php echo ($val); ?></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
        </div>-->
    </div>
    <?php if(is_array($lines)): $i = 0; $__LIST__ = $lines;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$line): $mod = ($i % 2 );++$i;?><a href="<?php echo U('line/info');?>/id/<?php echo ($line["id"]); ?>" target="_blank">
            <div class="item">
                ...
                <?php if(empty($line['check_price'])): ?><p>核算中</p>
                <?php else: ?>
                	<p>¥ <span><?php echo ($line["start_price_adult"]); ?></span>元起</p><?php endif; ?>
                <a type="button" class="btn btn-black" href="<?php echo U('line/info');?>/id/<?php echo ($line["id"]); ?>" target="_blank">更多内容</a>
            </div>
...
//		}
		...
		$.post('<?php echo U("line/xiezhenlvxing");?>', jsonData, function(data){
