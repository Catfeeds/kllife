...
//						});
	...
	<div class="page-title">
		...
		<div class="title-env">
			...
			<p class="description"><?php echo C('CONTENT_DES');?></p>
		</div>
	...
	<section class="gallery-env">
		...
		<div class="row">
			...
			<div class="col-sm-9 gallery-right">
				...
				<!-- Album Images -->
				<div class="album-images row">
					<?php if(is_array($ImageList)): $i = 0; $__LIST__ = $ImageList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$image): $mod = ($i % 2 );++$i;?><!-- Album Image -->
						<div class="col-md-3 col-sm-4 col-xs-6">
							<div class="album-image" data-id="<?php echo ($image["id"]); ?>">
								<a href="#" class="thumb my_pic_border">
									<img src="<?php echo ($image["path"]); ?>" class="img-responsive my_pic" />
								</a>
								
								<a href="#" class="name">
									<span><?php echo ($image["title"]); ?></span>
									<em><?php echo ($image["create_time"]); ?></em>
								</a>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			// 初始化页码
			initPageNum(parseInt('<?php echo ($PageCount); ?>'), 0);
...
//					alert($(imgRoot).length);
		...
		function _ajaxDeleteImages(){
			...
			$.post('<?php echo U("image/delete");?>', {ids: deleteIds}, function(data){
				if (data.result.errno == 0) {
					filterImages();
				} else {
					alert(data.result.message);
		...
		function _ajaxEditImages(pIds, data){
			...
			data['ids'] = editIds;
			$.post('<?php echo U("image/edit");?>', data, function(backData){
				if (backData.result.errno == 0) {
					// 改动页面图片信息
