...
<style>
			...
			<section class="profile-env">
				...
				<form method="post" action="" class="profile-post-form">
					...
					<div class="main_type">
						...
						<?php if(is_array($question_types)): $qt_key = 0; $__LIST__ = $question_types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$qt): $mod = ($qt_key % 2 );++$qt_key; if($qt['type_key'] != 'question_type_line'): ?><label data-id="<?php echo ($qt["id"]); ?>" style="float: left;">
									<input type="radio" name="question_type" class="cbr cbr-info">
									<?php echo ($qt["type_desc"]); ?>
					...
					<div class="qinglvpai_type" style="width: 900px;">
						...
						<?php if(is_array($question_qinglvpai_types)): $qt_qlp_key = 0; $__LIST__ = $question_qinglvpai_types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$qt_qlp): $mod = ($qt_qlp_key % 2 );++$qt_qlp_key; if($qt_qlp['type_key'] != 'question_qinglvpai_type_line'): ?><label data-id="<?php echo ($qt_qlp["id"]); ?>" style="float: left;">
									<input type="radio" name="question_type" class="cbr cbr-info">
									<?php echo ($qt_qlp["type_desc"]); ?>
								</label>&nbsp;&nbsp;&nbsp;&nbsp;<?php endif; endforeach; endif; else: echo "" ;endif; ?>
					</div>
				...
				<section class="user-timeline-stories">
					...
					<?php if(is_array($questions)): $i = 0; $__LIST__ = $questions;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$quest): $mod = ($i % 2 );++$i;?><hr class="separator-line"/>
					
						<!-- Timeline Story Type: Status -->
						<article class="timeline-story" data-id="<?php echo ($quest["id"]); ?>" data-type-id="<?php echo ($quest["type_id"]); ?>" data-line-id="{quest.line_id}">
							...
							<header>							
								<!-- Story Content Wrapped inside Paragraph -->
								<p class="hover_content" style="margin-bottom: 30px;">									
									<span class="content"><?php echo ($quest["content"]); ?></span><br />
									<i class="content_edit question">[编辑]</i>
									<i class="content_delete question">[删除]</i>
								</p>
								
								<p class="question_line">
									[<?php echo ($quest["type_id_data"]["type_desc"]); ?>]<?php if($quest['is_line'] == 1): ?>-- <?php echo ($quest["line"]["title"]); ?>(<?php echo ($quest["line"]["id"]); ?>)<?php endif; ?>
								</p>
								...
								<div class="story-options-links">
									...
									<a href="#">
										<i class="linecons-heart"></i>
										点赞数 
										<span class="like_count">(<?php echo ($quest["agree"]); ?>)</span>
									</a>
									
									<a href="#">
										<i class="linecons-comment"></i>
										评论数
										<span class="answer_count">(<?php echo ($quest["answer_count"]); ?>)</span>
									</a>
									
									<span class="sign_name" style="float: right;">
									
										<time style="float: right;"><?php echo ($quest["create_time"]); ?></time>
										
										<span class="sign_name" style="float: right;">[<?php echo ($quest["account_id_data"]["account_type"]["type_desc"]); ?>]<?php echo ($quest["account_id_show_name"]); ?></span>&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<div class="answer-content">
								...
								<ul class="list-unstyled story-comments">
									...
									<?php if(is_array($quest["answers"])): $i = 0; $__LIST__ = $quest["answers"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$answer): $mod = ($i % 2 );++$i;?><li>
										
										<div class="story-comment" data-id="<?php echo ($answer["id"]); ?>">
											
											<div class="story-comment-content">
											
												<a href="#" class="story-comment-user-name">														
													<span class="answer_name">[<?php echo ($answer["account_id_data"]["account_type"]["type_desc"]); ?>]<?php echo ($answer["account_id_show_name"]); ?></span>														
													<time><?php echo ($answer["create_time"]); ?></time>
												</a>
												
												<p class="hover_content" style="margin-bottom: 30px;">
													<span class="answer_content"><?php echo ($answer["content"]); ?></span><br />
													<i class="content_edit answer">[编辑]</i>
	...
	<!--翻页脚本-->
	<script src="/source/Static/home/js/page.js"></script>	
	<link rel="stylesheet" type="text/css" href="/source/Static/back/wysihtml5/lib/css/bootstrap.min.css"></link>
	<link rel="stylesheet" type="text/css" href="/source/Static/back/wysihtml5/lib/css/prettify.css"></link>
	<link rel="stylesheet" type="text/css" href="/source/Static/back/wysihtml5/src/bootstrap-wysihtml5.css"></link>
	
	<script src="/source/Static/back/wysihtml5/lib/js/wysihtml5-0.3.0.js"></script>
	<script src="/source/Static/back/wysihtml5/lib/js/prettify.js"></script>
	<script src="/source/Static/back/wysihtml5/lib/js/bootstrap.min.js"></script>
	<script src="/source/Static/back/wysihtml5/src/bootstrap-wysihtml5.js"></script>
	...
	<script type="text/javascript">
		...
		// 初始化翻页
		constructionPage('.page-list', 1, '<?php echo ($page_count); ?>', changePage, true);
		
		// 初始化回答问题框
		$('.user-timeline-stories').find('.timeline-story').each(function(i,ev){
			$(this).find('.answer_reply_textarea').wysihtml5({
				size: 'white',
				stylesheets: "/source/Static/back/wysihtml5/lib/css/wysiwyg-color.css",
			});	
		});
