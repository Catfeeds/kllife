...
<style>
	...
	.subject_preview { position: absolute; z-index: 99; bottom: 0px; right: 76px; }
	.update_cache { position: absolute; z-index: 99; bottom: 0px; right: 132px; }
	...
	<div class="row">		
		...
		<div class="subject-information-img template_subject hidden_ctrl subject_root" data-id="<?php echo ($sub["id"]); ?>">
		...
		<div id="content">
			...
			<div class="subject-list">
				...
				<?php if(is_array($subjects)): $i = 0; $__LIST__ = $subjects;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><div class="subject-information-img subject_root" data-id="<?php echo ($sub["id"]); ?>">
						<a class="detail" href="<?php echo U('line/subject');?>/type/edit/id/<?php echo ($sub["id"]); ?>" target="_blank"></a>
						
						<div class="subject-move">
							<input class="sort" type="text" maxlength="4" value="<?php echo ($sub["sort"]); ?>"/>
							<button class="btn btn-secondary btn_sort">排序</button>
						</div>
						
						<div class="subject-information-img-left">
							<img class="banner" src="<?php echo ($sub["banner"]); ?>" alt="">
						</div>
						<div class="subject-information-img-right">
							<h3 class="title"><?php echo ($sub["title"]); ?>(No.<?php echo ($sub["id"]); ?>)</h3>
							<p class="desc"><?php echo ($sub["desc"]); ?></p>
							<p class="keywords"><?php echo ($sub["keywords"]); ?></p>
							<i class="time"><?php echo ($sub["create_time"]); ?></i>
							<button class="btn btn-danger subject_delete" data-id="<?php echo ($sub["id"]); ?>">删除</button>
						</div>
						<button class="btn btn-secondary subject_preview" data-id="<?php echo ($sub["id"]); ?>">预览</button>
						<button class="btn btn-secondary update_cache" data-set="cache_set_subject_<?php echo ($sub["id"]); ?>" data-op="LIKE">更新缓存</button>			
	...
	<div class="footer" style="text-align: right; margin-top: 20px;">						
		<a href="<?php echo U('line/subject');?>/type/edit" target="_blank" class="btn btn-secondary">创建专题</a>	
	</div>
	<!--面板内容结束-->
	...
	<script>
		...
		$('.subject_preview').click(function(){
			...
			var w = window.open();
			$.post('<?php echo U("line/subject");?>',{op_type:'preview_url',id: sid}, function(data){
				if (data.result.errno != 0) {
					toastr.error(data.result.message);
				}
		...
		$('.subject_delete').click(function(){
			...
			$.post('<?php echo U("line/subject");?>', {op_type:'delete', id:$(this).attr('data-id')}, function(data){
				if (data.result.errno == 0) {
					$(rootObj).remove();
				} else {
					alert(data.result.message);
		...
		// 分页初始化
		constructionPage($('.list-page'), 1, '<?php echo ($page_count); ?>', loadMoreData, true);
				...
				if (data.result.errno == 0) {
					...
					if (data.ds != null && data.ds != 'undefined') {				
						...
						for (var i = 0; i < data.ds.length; i ++){
							...
							var vhref = '<?php echo U("line/subject");?>/type/edit/id/'+d.id;
