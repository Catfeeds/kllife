									...
									<div class="input-group">
										...
										<script type="text/javascript">	
											$(document).ready(function(){
												...
												if ('<?php echo ($admin["station_id"]); ?>' != '1') {
													cds += '|station_id|=|<?php echo ($admin["station_id"]); ?>|AND||0';
												}
									...
									<div class="input-group">
										...
										<script type="text/javascript">	
											$(document).ready(function(){
												...
												if ('<?php echo ($admin["station_id"]); ?>' != '1') {
													cds += '|station_id|=|<?php echo ($admin["station_id"]); ?>|AND';
												}
								...
								<div class="col-sm-3">
									<div class="input-group">
										...
										<select class="form-control" name="" id="team_from">
											<option value="-1">不限</option>
											<?php if(!empty($TeamFrom)): if(is_array($TeamFrom)): $i = 0; $__LIST__ = $TeamFrom;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$from): $mod = ($i % 2 );++$i;?><option value="<?php echo ($from["id"]); ?>"><?php echo ($from["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
										</select>
									</div>
								</div>
								...
								<div class="col-sm-3">
									<div class="input-group">
										...
										<select class="form-control" name="" id="team_state">
											<option value="-1">不限</option>
											<?php if(!empty($TeamState)): if(is_array($TeamState)): $i = 0; $__LIST__ = $TeamState;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$state): $mod = ($i % 2 );++$i;?><option value="<?php echo ($state["id"]); ?>"><?php echo ($state["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
										</select>
									</div>
								</div>
								...
								<div class="col-sm-3">
									<div class="input-group">
										...
										<select class="form-control" name="" id="team_result">
											<option value="-1">不限</option>
											<?php if(!empty($TeamResult)): if(is_array($TeamResult)): $i = 0; $__LIST__ = $TeamResult;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$result): $mod = ($i % 2 );++$i;?><option value="<?php echo ($result["id"]); ?>"><?php echo ($result["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
										</select>
									</div>
								</div>
...
<script type="text/javascript">
	...
	function setTypeList(objId) {
		$.post('<?php echo U("team/disposeAdmin");?>', {op:'list'}, function(data){
			if (data.result.errno == 0) {
...
//		alert(nTd+'|'+JSON.stringify(sData)+'|'+JSON.stringify(oData)+'|'+iRow+'|'+iCol);
		...
		html += '<a href="'+'<?php echo U("team/info");?>/id/'+id+'" class="btn btn-secondary btn-sm btn-icon icon-left" target="_blank">详细信息</a>';
	...
	function _assginAjax(ds) {		
		...
		$.post('<?php echo U("team/disposeAdmin");?>', jsonData, function(data){
			if (data.result.errno != 0) {
				toastr.error(data.result.message);
			} else {				
		    	hideModalDataEdit();
	...
	function _editFun(aid) {
		$.getJSON("<?php echo U('team/info');?>", {op:'team_info', id:aid}, function(data){
			if (data.result.errno == 0) {			
				location.href=data.jumpUrl;	
		        myTable.fnReloadAjax(myTable.fnSettings());
			} else {
	...
	function _deleteFun(aid) {		
		...
		$.post('<?php echo U("team/list");?>', jsonData, function(data){
			if (data.result.errno != 0) {
				toastr.error(data.result.message);
			} else {				
		    	myTable.fnReloadAjax();
