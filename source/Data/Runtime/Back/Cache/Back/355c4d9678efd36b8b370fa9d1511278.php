...
//						});
					...
					<li <?php echo C('MENU_ACTIVE')=='statistics' ? 'class="active opened"' : '';?>>
						...
						<ul>
							...
							<li <?php echo C('MENU_CURRENT')=='statistics_day' ? 'class="active"' : '';?>>
								<a href="<?php echo U('statistics/day');?>"><span class="title">线路统计</span></a>
							</li>
							...
							<div class="row">						
								<div class="col-md-4">
									<div class="input-group">
										...
										<input type="text" class="form-control daterange data_stats_date_range" readonly  data-format="YYYY-MM-DD"  data-separator=" / " value="<?php echo ($today); ?> / <?php echo ($today); ?>" />
									</div>
								</div>
							</div>
		...
		<div class="col-sm-12">
			<div class="panel panel-default">
				<div class="panel-heading">
					...
					<div class="panel-options">
						<a href="<?php echo U('statistics/export');?>/type/order" class="btn btn-secondary btn-xs" style="color: #fff;" target="_blank">导出</a>
						<a href="#" data-toggle="panel">
							<span class="collapse-icon">–</span>
							<span class="expand-icon">+</span>
						</a>
				...
				<div class="panel-body" style="padding: 20px 20px; position: relative;">	
					<span class="font-title">线路产品综合统计</span>
					<table id="table_day" class="table">
						<thead>
							<tr>
								<th class="font-table-th" style="width: 130px;">线路批次</th>
								<th class="font-table-th" style="width: 130px;">状态</th>
								<th class="font-table-th" style="width: 130px;">累计订单数</th>
								<th class="font-table-th" style="width: 130px;">累计报名人数</th>
								<th class="font-table-th" style="width: 130px;">当天订单数</th>
								<th class="font-table-th" style="width: 130px;">当天报名人数</th>
								<th class="font-table-th" style="width: 130px;">主站订单</th>
								<th class="font-table-th" style="width: 130px;">缴费人数</th>
								<th class="font-table-th" style="width: 130px;">渠道订单</th>
								<th class="font-table-th" style="width: 130px;">渠道缴费人数</th>
							</tr>
						</thead>
						<tbody></tbody>
					</table>
					...
					<span class="font-title">线路产品订单统计</span>
					...
					<span class="font-title">线路产品参团人统计</span>
	...
	<script src="/source/Static/assets/js/devexpress-web-14.1/js/globalize.min.js"></script>
	<script src="/source/Static/assets/js/devexpress-web-14.1/js/dx.chartjs.js"></script>
	<script type="text/javascript">	
		// 统计线路发生改变时
		...
		function getStatsLineData() {
			var vtype = $('.data_stats_line_type').val();
			var linestr = getSelect2Value('.data_stats_line', ['id', 'title', 'online']);
		...
		$('.btn-append-stats').click(function(){
			...
			if (selData == null) {
				toastr.warning('您可以不选择批次但至少选择一个产品');
				return false;
			}				
			var ds = JSON.stringify(selData);
			var showText = selData.title;
			var idkey = 'line_'+selData.type+'_'+selData.line.id;
			if (selData.batch != null) {
				idkey += 'batch_'+selData.batch.id;
			}
		...
		$('.btn-start-stats').click(function(){			
			...
			$.post('<?php echo U("statistics/day");?>', jsonData, function(data){
				if (data.result.errno == 0) {
					if (data.ds != null) {
						// 滚动到结果
       					$('body').animate({scrollTop:$('#table_day').offset().top}, 500);
						$('#table_day').find('tbody').children().remove();
						$('#table_order').find('thead tr').children().remove();
						$('#table_member').find('thead tr').children().remove();
						$('#table_order').find('thead tr').append('<th class="font-table-th" style="width: 130px;">线路批次</th>');
						$('#table_member').find('thead tr').append('<th class="font-table-th" style="width: 130px;">线路批次</th>');
						$('#table_order').find('tbody').children().remove();
						$('#table_member').find('tbody').children().remove();
						...
						for (var i=0; i<data.ds.length; i++) {
							...
							var vhtml = '<tr">'+
										'	<td>'+d.title+'</td>'+
										'	<td>'+(d.line.online == 0 ? '下架' : '上架')+'</td>'+
										'	<td>'+d.sum_order+'</td>'+
										'	<td>'+d.sum_member+'</td>'+
										'	<td>'+d.day_order+'</td>'+
										'	<td>'+d.day_member+'</td>'+
										'	<td>'+d.main_station_order+'</td>'+
										'	<td>'+d.main_station_pay+'</td>'+
										'	<td>'+d.channel_order+'</td>'+
										'	<td>'+d.channel_member+'</td>'+
										'</tr>';
							$('#table_day').find('tbody').append(vhtml);
							
							if (d.stats_channels != null) {								
								// 表标题
								if (addedHead == false) {
									for (k in d.stats_channels) {
										var channel = d.stats_channels[k];
										$('#table_order').find('thead tr').append('<th class="font-table-th" style="width: 130px;">'+channel.type_desc+'</th>');
										$('#table_member').find('thead tr').append('<th class="font-table-th" style="width: 130px;">'+channel.type_desc+'</th>');
									}
									addedHead = true;
								}
								
								// 表数据
								var orderHtml = '<tr><td>'+d.title+'</td>';
								var memberHtml = '<tr><td>'+d.title+'</td>';
								for (k in d.stats_channels) {
									var channel = d.stats_channels[k];							
									orderHtml += '<td>'+channel.order_count+'</td>';
									memberHtml += '<td>'+channel.member_count+'</td>';
								}
								orderHtml += '</tr>';
								memberHtml += '</tr>';
								$('#table_order').find('tbody').append(orderHtml);
								$('#table_member').find('tbody').append(memberHtml);
							}							
						}
					}
				} else {
