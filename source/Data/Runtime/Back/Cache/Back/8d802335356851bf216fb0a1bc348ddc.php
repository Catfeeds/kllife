						...
						<div class="create-write" data-id="">
							...
							<div class="row">
								<div class="col-md-6">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">公司名称</i>
										</div>
										<input type="text" class="form-control data_company" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
								...
								<div class="col-md-3">
									<div class="input-group">
										...
										<input type="text" class="form-control data_contacts_mob" maxlength="11">
									</div>
								</div>
							</div>
				...
				<div class="panel-body">						
					...
					<div class="issuing-list">
						...
						<div class="issuing-list-search">
							<div class="input-group">
								<input type="text" class="form-control" placeholder="搜索内容可以是编号、所属线路、公司名称、联系人姓名、联系人手机">
							...
							<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
								<thead>
									<tr role="row" class="head">
										...
										<th class="sorting" rowspan="1" colspan="1" style="width: 200px;">公司名称</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 100px;">联系人</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 100px;">联系人电话</th>
								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ds): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($ds["id"]); ?>">
										<?php else: ?>
											...
											<td><?php echo ($ds["contacts"]); ?></td>
											<td><?php echo ($ds["contacts_mob"]); ?></td>
...
<script type="text/javascript">
		...
		$.post('<?php echo U("financial/source");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.op != null && data.op == 'create') {
					location.href = '<?php echo U("financial/source");?>/src/insurance';
				} else {
					if (data.ds != null) {
						var d = data.ds;
						var trObj = $('tr[data-id="'+d.id+'"]');
						$(trObj).find('td:eq(1)').html(d.name);
						$(trObj).find('td:eq(2)').html(d.contacts);
						$(trObj).find('td:eq(3)').html(d.contacts_mob);
	...
	var itemObjs = {};
	function itemData(){
		var insuranceId = $(this).closest('tr').attr('data-id');
		var itemObj = itemObjs['item_'+insuranceId];
		if (itemObj == null) {
			itemObj = new ModalCJItem(this, 'insurance', insuranceId);
			itemObjs['item_'+insuranceId] = itemObj;
		}	
		itemObj.showItems();
	}
	...
	function editData(){
		...
		$.post('<?php echo U("financial/source");?>', {op_type:'find', src:'insurance', id:did}, function(data){
			if (data.ds != null) {
				var d = data.ds;        		
				$('.create-write').attr('data-id', d.id);
				$('.data_company').val(d.name);
	...
	function deleteData() {
		...
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("financial/source");?>', {op_type:'delete', src: 'insurance', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
					$(trObj).remove();
	...
	function findPageData(p) {			
		...
		if (searchval != '') {
			jsonData['cdsstr'] = 'id|=|'+searchval+'|OR|line|LIKE|%'+searchval+'%|OR|name|LIKE|%'+searchval+'%|OR|contacts|LIKE|%'+searchval+'%|OR|contacts_mob|LIKE|%'+searchval+'%|OR|bank_card|LIKE|%'+searchval+'%|OR|bank_address|LIKE|%'+searchval+'%|OR';
		}
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">'+
								...
								'	<td>'+d.name+'</td>'+
								'	<td>'+d.contacts+'</td>'+
								'	<td>'+d.contacts_mob+'</td>'+
								'	<td>'+d.line_show+'</td>'+
								'	<td>收款人:'+d.payee+'<br />卡号:'+d.bank_card+'<br />开户行:'+d.bank_address+'<br />结算方式:'+d.pay_type_show+'</td>'+
								'	<td>'+
								'		<a href="javascript:;" class="btn btn-secondary btn-sm btn-icon icon-left btn_item">价格项</a>'+
								'		<a href="javascript:;" class="btn btn-secondary btn-sm btn-icon icon-left btn_edit">编辑</a>'+
								'		<a href="javascript:;" class="btn btn-danger btn-sm btn-icon icon-left btn_del">删除</a>'+
