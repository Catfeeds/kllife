						...
						<div class="create-write" data-id="">
							...
							<div class="row">
								...
								<div class="col-md-5">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">资源所在地</i>
										</div>
				...
				<div class="panel-body">						
					...
					<div class="issuing-list">
						...
						<div class="issuing-list-search">
							<div class="input-group">
								<input type="text" class="form-control" placeholder="搜索内容可以是编号、所属线路、资源名称、资源地址、联系人、联系人电话、开户行">
							...
							<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
								<thead>
									<tr role="row" class="head">
										...
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">资源名称</th>
										...
										<th class="sorting" rowspan="1" colspan="1" style="width: 100px;">负责计调</th>
								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ds): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($ds["id"]); ?>">
										<?php else: ?>
											...
											<?php $admin = json_decode($ds['admin'],true); ?>
...
<script type="text/javascript">
		...
		$.post('<?php echo U("financial/source");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.op != null && data.op == 'create') {
					location.href = '<?php echo U("financial/source");?>/src/source';
				} else {
					if (data.ds != null) {
						var d = data.ds;
						var trObj = $('tr[data-id="'+d.id+'"]');
						$(trObj).find('td:eq(1)').html(d.name);
						$(trObj).find('td:eq(2)').html(d.contacts+'<br />'+d.contacts_mob);
						$(trObj).find('td:eq(3)').html(d.address);
						if (d.admin != null && d.admin != 'undefined') {
							var admin = $.parseJSON(d.admin);
							$(trObj).find('td:eq(4)').html(admin.account);
						}
						$(trObj).find('td:eq(5)').html(d.line_show);
						$(trObj).find('td:eq(6)').html('收款人:'+d.payee+'<br />卡号:'+d.bank_card+'<br />开户行:'+d.bank_address+'<br />结算方式:'+d.pay_type_show);
						$(trObj).find('td:eq(7)').html(d.beizhu);
					}
					clearInfo();
	...
	function itemData(){
		...
		var itemObj = itemObjs['item_'+sourceId];
		if (itemObj == null) {
			itemObj = new ModalCJItem(this, 'source', sourceId);
			itemObjs['item_'+sourceId] = itemObj;
		}	
		itemObj.showItems();
	}
	...
	function editData(){
		...
		$.post('<?php echo U("financial/source");?>', {op_type:'find', src:'source', id:did}, function(data){
			if (data.ds != null) {
				var d = data.ds;
	...
	// 删除信息
	function deleteData() {
		if (!confirm('您确定要删除这条资源信息?')) {
			return false;
		}
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("financial/source");?>', {op_type:'delete', src: 'source', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
					$(trObj).remove();
	...
	function findPageData(p) {			
		...
		if (searchval != '') {
			jsonData['cdsstr'] = 'id|=|'+searchval+'|OR|line|LIKE|%'+searchval+'%|OR|name|LIKE|%'+searchval+'%|OR|address|LIKE|%'+searchval+'%|OR|admin|LIKE|%'+searchval+'%|OR|contacts|LIKE|%'+searchval+'%|OR|contacts_mob|LIKE|%'+searchval+'%|OR|payee|LIKE|%'+searchval+'%|OR|bank_card|LIKE|%'+searchval+'%|OR|bank_address|LIKE|%'+searchval+'%|OR';
		}
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">';
							...
							if (d.admin != null && d.admin != 'undefined') {
								var admin = $.parseJSON(d.admin);
								$(trObj).find('td:eq(5)').html(admin.account);
								vhtml +='	<td>'+payType.account+'</td>';
							} else {
								vhtml +='	<td></td>';
							}
