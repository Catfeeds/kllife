								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ds): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($ds["id"]); ?>">
										<?php else: ?>
											...
											<?php $payType = json_decode($ds['pay_type'], true); $conds = ''; if (!empty($ds['decorate_time'])) { $conds .= '装修时间:'.$ds['decorate_time']; } if (!empty($ds['centre'])) { $conds .= empty($conds) ? '市中心' : ',市中心'; } if (!empty($ds['bus_station'])) { $conds .= empty($conds) ? '大巴站点' : ',大巴站点'; } if (!empty($ds['receipt'])) { $conds .= empty($conds) ? '发票' : ',发票'; } if (!empty($ds['led_screen'])) { $conds .= empty($conds) ? 'LED屏幕' : ',LED屏幕'; } if (!empty($ds['parking_lot'])) { $conds .= empty($conds) ? '停车场' : ',停车场'; } if (!empty($ds['elevator'])) { $conds .= empty($conds) ? '电梯' : ',电梯'; } if (!empty($ds['hot_water'])) { $conds .= empty($conds) ? '24小时热水' : ',24小时热水'; } ?>
											<td><?php echo ($ds["id"]); ?></td>
											<td><?php echo ($ds["name"]); ?></td>
											<td><?php echo ($ds["city"]); ?></td>
											<td><?php echo ($ds["tel"]); ?></td>
											<td><?php echo ($ds["fax"]); ?></td>
											<td><?php echo get_star($ds['star_level']);?></td>
											<td><?php echo ($ds["line_show"]); ?></td>
											<td><?php echo ($conds); ?></td>
											<td><?php echo ($ds["leading_official"]); ?><br><?php echo ($ds["leading_official_mob"]); ?></td>
...
<script type="text/javascript">
		...
		$.post('<?php echo U("financial/source");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.op != null && data.op == 'create') {
					location.href = '<?php echo U("financial/source");?>/src/hotel';
				} else {
					if (data.ds != null) {
						var d = data.ds;
	...
	function editData(){
		...
		$.post('<?php echo U("financial/source");?>', {op_type:'find', src:'hotel', id:did}, function(data){
			if (data.ds != null) {
				var d = data.ds;
	...
	function deleteData() {
		...
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("financial/source");?>', {op_type:'delete', src: 'hotel', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
					$(trObj).remove();
