				...
				<div class="panel panel-color panel-content">
                    <table id="test" class="table aspTable lcnp" cellpadding="0" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th width="10%">权限</th>
                            	<?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><th name='group'><?php echo $group['my.type_desc'];?></th><?php endforeach; endif; else: echo "" ;endif; ?>
                            </tr>
                        </thead>
                    </table>   
					<br /><br /><br />
...
<script type="text/javascript">
	...
	function initDataTable() {	
		var vTable = $(".aspTable").dataTable({
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "<?php echo U('grant/group');?>",
...
//		console.log('row:'+iRow+',col:'+iCol+',grant_id:'+oData['id']+',group_id:'+oData[key]['group_id']);
	...
	function _changeFun(){
		...
		var obj = $(this);
		$.post('<?php echo U("grant/group");?>', jsonData, function(data){
			if (data.result.errno != 0) {
				alert(data.result.message);
