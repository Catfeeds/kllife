							...
							<div class="col-sm-3">
								...
								<div class="input-group">
									...
									<select class="form-control" name="" id="order_from">
										...
										<?php if(!empty($OrderFrom)): if(is_array($OrderFrom)): $i = 0; $__LIST__ = $OrderFrom;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$from): $mod = ($i % 2 );++$i;?><option value="<?php echo ($from["id"]); ?>"><?php echo ($from["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
							...
							<div class="col-sm-3">
								...
								<div class="input-group">
									...
									<select class="form-control" name="" id="order_state">
										...
										<?php if(!empty($OrderState)): if(is_array($OrderState)): $i = 0; $__LIST__ = $OrderState;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$os): $mod = ($i % 2 );++$i;?><option value="<?php echo ($os["id"]); ?>"><?php echo ($os["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
							...
							<div class="col-sm-3">
								...
								<div class="input-group">
									...
									<select class="form-control" name="" id="station">
										...
										<?php if(!empty($Stations)): if(is_array($Stations)): $i = 0; $__LIST__ = $Stations;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$station): $mod = ($i % 2 );++$i;?><option value="<?php echo ($station["id"]); ?>"><?php echo ($station["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
...

											{
												...
												var lineObj = $("#line_article").select2({
													...
													formatSelection: function(data) { 			
														...
														$.post('<?php echo U("order/activityFind");?>', {aid:data.id, new_line:newLine}, function(resp){
...
//	    		myTable.fnReloadAjax(myTable.fnSettings());
	...
	function initFileImport() {
		...
		$("#file_import_order").fileupload({
			...
			url: "<?php echo U('order/importExcel');?>",
	...
	function exportData() {
		...
		location.href = '<?php echo U("order/exportExcel");?>';
	...
	function statisticsOrder() {
		...
		$.post('<?php echo ($ajaxReqUrl); ?>', jsonData, function(data){
...
//		alert(nTd+'|'+JSON.stringify(sData)+'|'+JSON.stringify(oData)+'|'+iRow+'|'+iCol);
		...
		if ('<?php echo ($grant_review); ?>' == true) {
			...
			if (oData['zhuangtai_show_btn_account'] == 'show') {
				...
				html += '<a href="'+'<?php echo U("review/request");?>/oid/'+id+'" class="btn btn-warning btn-sm btn-icon icon-left" target="_blank">线 下 付 款</a>';

			}

		}
		
		html += '<a href="'+'<?php echo U("order/info");?>/id/'+id+'" class="btn btn-secondary btn-sm btn-icon icon-left mybtn" target="_blank">订 单 详 细</a>';	
