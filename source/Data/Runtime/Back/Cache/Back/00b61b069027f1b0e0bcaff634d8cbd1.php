...
//						});
					...
					<li <?php echo C('MENU_ACTIVE')=='statistics' ? 'class="active opened"' : '';?>>
						...
						<ul>
							...
							<li <?php echo C('MENU_CURRENT')=='statistics_line' ? 'class="active"' : '';?>>
								<a href="<?php echo U('statistics/line');?>"><span class="title">线路统计</span></a>
							</li>
	...
	<style>
		...
		.btn-save-coupon { position : absolute; width:100px; right: 180px; bottom: 20px; background-color: #68b828; color: #fff; }
		.btn-clear-coupon { position : absolute; width:100px; right: 50px; bottom: 20px; background-color: #68b828; color: #fff; }
			...
			<div class="panel panel-default">
				...
				<!--在此地编辑内容-->
					...
					<div class="create-coupon">
						<div class="create-coupon-write" data-id="">
						
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">账号</i>
										</div>
										<input type="text" class="form-control data_username" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">密码</i>
										</div>
										<input type="password" class="form-control data_password">
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
									...
									<div class="input-group">
										...
										<div class="col-sm-10 input-select">
											<select class="form-control data_type">
												<?php if(is_array($user_types)): $i = 0; $__LIST__ = $user_types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ut): $mod = ($i % 2 );++$i;?><option value="<?php echo ($ut["id"]); ?>"><?php echo ($ut["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
											</select>
										</div>
									</div>
								</div>
							...
							<div class="row">
								<div class="col-md-4">
									<div class="input-group">
										...
										<input type="number" class="form-control data_phone" maxlength="11">
									</div>
								</div>
								<div class="col-md-4">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">电子邮件</i>
										</div>
							...
							<div class="row">
								...
								<button class="btn btn-clear-coupon">重置</button>	
							</div>
					...
					<div class="issuing-coupon">
						...
						<div class="issuing-coupon-search">
							<div class="input-group">
								<input type="text" class="form-control" placeholder="搜索内容可以是编号、用户名、姓名、手机、邮箱">
							...
							<table class="table table-bordered table-striped dataTable no-footer" id="table_user" role="grid" aria-describedby="example-2_info">
								<thead>
									<tr role="row" class="head">
										...
										<th class="sorting" tabindex="3" rowspan="1" colspan="1" style="width: 100px;">手机</th>
										<th class="sorting" tabindex="4" rowspan="1" colspan="1" style="width: 130px;">邮箱</th>
										<th class="sorting" tabindex="5" rowspan="1" colspan="1" style="width: 80px;">类型</th>
										<th class="sorting" tabindex="6" rowspan="1" colspan="1" style="width: 130px;">订单(已完成/未完成)</th>
										<th class="sorting" tabindex="7" rowspan="1" colspan="1" style="width: 130px;">成人/小孩人数</th>
								...
								<tbody class="middle-align coupon-tbody">
									<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$u): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($u["id"]); ?>">
										<?php else: ?>
											<tr role="row" class="even" data-id="<?php echo ($u["id"]); ?>"><?php endif; ?>
											<td><?php echo ($u["id"]); ?></td>
											<td><?php echo ($u["username"]); ?></td>
											<td><?php echo ($u["name"]); ?></td>
											<td><?php echo ($u["phone"]); ?></td>
											<td><?php echo ($u["email"]); ?></td>
											<td><?php echo ($u["type_id_data"]["type_desc"]); ?></td>
											<td><?php echo ($u["order_complated"]); ?>/<?php echo ($u["order_uncomplated"]); ?></td>
											<td>已完成：<?php echo ($u["member_adult_complated"]); ?>/<?php echo ($u["member_child_complated"]); ?><br />未完成：<?php echo ($u["member_adult_uncomplated"]); ?>/<?php echo ($u["member_child_uncomplated"]); ?></td>
											<td><?php if($u['forbidden'] == 1): ?>已禁用<?php else: ?>正常<?php endif; ?></td>
											<td>
												<a href="#" class="btn btn-secondary btn-sm btn-icon icon-left btn_modify_info">编辑</a>
												<a href="#" class="btn btn-secondary btn-sm btn-icon icon-left btn_modify_password">更改密码</a>
												<?php if($u['forbidden'] == 1): ?><a href="#" data-set='forbidden' data-val="<?php echo ($u["forbidden"]); ?>" class="btn btn-danger btn-sm btn-icon icon-left btn_modify">启用</a>
												<?php else: ?>
													<a href="#" data-set='forbidden' data-val="<?php echo ($u["forbidden"]); ?>" class="btn btn-secondary btn-sm btn-icon icon-left btn_modify">禁用</a><?php endif; ?>
											</td>
...
<script type="text/javascript">	
	...
	// 清除信息
	function clearInfo() {
		$('.create-coupon-write').attr('data-id', '');
		$('.data_username').val('');
		$('.data_password').val('');
		$('.data_name').val('');
	...
	$('.btn-clear-coupon').click(function(){
		clearInfo();
	});
	...
	$('.btn-save-coupon').click(function(){
		var jsonData = {
			op_type: 'save',
			id: $('.create-coupon-write').attr('data-id'),
			username: $('.data_username').val(),
			password: $('.data_password').val(),
			name: $('.data_name').val(),
			type_id: $('.data_type').val(),
			phone: $('.data_phone').val(),
			email: $('.data_email').val(),
		}	
		
		$.post('<?php echo U("user/distribute");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.type != null && data.type == 'create') {
					location.href = '<?php echo U("user/distribute");?>';
				} else {
					if (data.ds != null) {
						var d = data.ds;
						var trObj = $('tr[data-id="'+data.ds.id+'"]');
						$(trObj).find('td:eq(1)').html(d.username);
						$(trObj).find('td:eq(2)').html(d.name);
						$(trObj).find('td:eq(3)').html(d.phone);
						$(trObj).find('td:eq(4)').html(d.email);
						$(trObj).find('td:eq(5)').html(d.type_id_data.type_desc);
					}
					clearInfo();
				}
			} else {
	...
	// 编辑信息
	$('.btn_modify_info').click(function(){
		var trObj = $(this).closest('tr');
		var userId = $(trObj).attr('data-id');
		$.post('<?php echo U("user/distribute");?>', {op_type:'find', id:userId}, function(data){
			if (data.ds != null) {
				var d = data.ds;
				$('.create-coupon-write').attr('data-id', d.id);
				$('.data_username').val(d.username);
				$('.data_password').val(d.password);
				$('.data_name').val(d.name);
				$('.data_type').val(d.type_id).trigger('change');
				$('.data_phone').val(d.phone);
				$('.data_email').val(d.email);
			}
			if (data.result != null) {
				toastr.error(data.result.message);
			}
	...
	function modifyPassword(ds) {
		...
		console.log(JSON.stringify(jsonData));
		$.post('<?php echo U("user/distribute");?>',jsonData, function(backdata,status) {
	        if (backdata['result']['errno'] == 0) {
	            hideModalDataEdit();
	            alert('修改密码成功');
	        } else {
...
//			alert(data.result.message);
	...
	// 搜索用户数据
	constructionPage($('.list-page'), 1, '<?php echo ($page_count); ?>', findUser, true);	
	function findUser(p) {			
		var jsonData = {
			op_type:'list',
			page: p-1,
		...
		var searchval = $('.issuing-coupon-search').find('input').val();
		if (searchval != '') {
			jsonData['cdsstr'] = 'id|=|'+searchval+'|OR|username|LIKE|%'+searchval+'%|OR|name|LIKE|%'+searchval+'%|OR|phone|LIKE|%'+searchval+'%|OR|email|LIKE|%'+searchval+'%|OR';
		}
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">'+
								...
								'	<td>'+d.order_complated+'/'+d.order_uncomplated+'</td>'+
								'	<td>未完成：'+d.member_adult_complated+'/'+d.member_child_complated+'<br />已完成：'+d.member_adult_uncomplated+'/'+d.member_child_uncomplated+'</td>'+
								'	<td>'+ forbidTd + '</td>'+
								'	<td>'+
								'		<a href="#" class="btn btn-secondary btn-sm btn-icon icon-left btn_modify_password">更改密码</a>'+
								'		<a href="#" data-set="forbidden" data-val="'+d.forbidden+'" class="btn '+forbidClass+' btn-sm btn-icon icon-left btn_modify">'+ forbidBtn + '</a>'+
								'	</td>'+
								'</tr>';
