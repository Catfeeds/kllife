...
<link rel="stylesheet" href="/source/Static/common/css/page.css">
<style type="text/css">
					...
					<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
						<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
							...
							<tbody class="middle-align list-tbody">
								<?php if(is_array($prices)): $i = 0; $__LIST__ = $prices;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($p["id"]); ?>">
									<?php else: ?>
										<tr role="row" class="even" data-id="<?php echo ($p["id"]); ?>"><?php endif; ?>
										<td><?php echo ($p["id"]); ?></td>
										<td><?php echo ($p["obj_type_data"]["type_desc"]); ?>-><?php echo ($p["obj_data"]["show_name"]); ?>-><?php echo ($p["item_data"]["name"]); ?></td>
										<td><?php echo ($p["show_name"]); ?></td>
										<td class="price"><?php echo ($p["price"]); ?></td>
										<td class="review"><?php echo ($p["review_type"]["type_desc"]); ?></td>
										<td><?php echo ($p["commit_admin"]["show_name"]); ?></td>
										<td><?php echo ($p["review_admin"]["show_name"]); ?></td>
										<td>
											<?php if($p['review_type']['type_key'] == 'reviewing' and empty($grant_review_price) == false): ?><a href="javascript:;" class="btn btn-danger btn-sm btn-icon icon-left btn_agree">同意</a>
												<a href="javascript:;" class="btn btn-warning btn-sm btn-icon icon-left btn_refuse">拒绝</a><?php endif; ?>
										</td>
...
<script src="/source/Static/common/js/page.js"></script>	
			<!-- Main Footer -->
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">';
							...
							vhtml +='	<td>';
							if (d.review_type.type_key == 'reviewing' && '<?php echo ($grant_review_price); ?>' == '1') {
