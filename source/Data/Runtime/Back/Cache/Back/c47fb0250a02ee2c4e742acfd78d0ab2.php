								...
								<tbody class="middle-align coupon-tbody">
									<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$u): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($u["id"]); ?>">
										<?php else: ?>
											...
											<td><?php echo get_sex($u.sex);?></td>
											<td><?php if($u['blacklist'] == 1): ?>已拉黑<?php else: ?>正常<?php endif; ?></td>
											...
											<td>
												...
												<?php if($u['blacklist'] == 1): ?><a href="#" data-set='blacklist' data-val="<?php echo ($u["blacklist"]); ?>" class="btn btn-danger btn-sm btn-icon icon-left btn_modify">恢复</a>
												<?php else: ?>
													<a href="#" data-set='blacklist' data-val="<?php echo ($u["blacklist"]); ?>" class="btn btn-secondary btn-sm btn-icon icon-left btn_modify">拉黑</a><?php endif; ?>
...
<script type="text/javascript">	
	...
	function modifyPassword(ds) {
		...
		console.log(JSON.stringify(jsonData));
		$.post('<?php echo U("user/center");?>',jsonData, function(backdata,status) {
	        if (backdata['result']['errno'] == 0) {
	            hideModalDataEdit();
	            alert('修改密码成功');
	        } else {
	...
	function modifyUserState() {
		...
		$.post('<?php echo U("user/center");?>', jsonData, function(data){
