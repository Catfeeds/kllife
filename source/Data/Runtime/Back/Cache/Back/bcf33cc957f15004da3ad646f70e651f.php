	...
	<script type="text/javascript">			
		...
		function bindTypeDataDropMenu(obj, typeKey, placeHolder, allowAddItem, bsearch, bmultiple, changeFunc) {	
			...
			if ( bsearch == null || bsearch == false ) {
				voption['minimumResultsForSearch'] = Infinity;
			} else {
...
<script type="text/javascript">
	...
	ModalFCJItem.prototype.confirmItem = function(e){
		...
		var param = {
			...
			'id': thisObj.item_id,
			'name': $(thisObj.root).find('#data_fitem_name').val(),
			'type': getSelect2Value($(thisObj.root).find('#data_fitem_type')),
			'stock': $(thisObj.root).find('#data_fitem_stock').val(),
			'room_area': $(thisObj.root).find('#data_fitem_room_area').val(),
			'window': $(thisObj.root).find('#data_fitem_window').val(),
			'air_conditioner': $(thisObj.root).find('#data_fitem_air').val(),
			'bed_width': getSelect2Value($(thisObj.root).find('#data_fitem_bed_type')),
			'wash_supplies': $(thisObj.root).find('#data_fitem_wash_supplies').prop('checked') ? 1 : 0,
			'bathroom_curtain': $(thisObj.root).find('#data_fitem_bathroom_curtain').prop('checked') ? 1 : 0,
			'double_breakfast': $(thisObj.root).find('#data_fitem_double_breakfast').prop('checked') ? 1 : 0,
			'append_bed': $(thisObj.root).find('#data_fitem_append_bed').prop('checked') ? 1 : 0,
			'wifi': $(thisObj.root).find('#data_fitem_wifi').prop('checked') ? 1 : 0,
			'necessary': $(thisObj.root).find('#data_fitem_necessary').prop('checked') ? 1 : 0,
			'is_free': $(thisObj.root).find('#data_fitem_free').prop('checked') ? 1 : 0,
			'is_recycle': $(thisObj.root).find('#data_fitem_recycle').prop('checked') ? 1 : 0,
			'beizhu': $(thisObj.root).find('#data_fitem_beizhu').val(),
		}
		
		if (thisObj.obj_type == null || thisObj.obj_id == null || param.name == '' || param.type == '') {
			toastr.warning('保存消费项的参数不足，可能缺少资源类型、资源对象、消费项名称、消费项类型其中的一项，所以价格不能保存');
			return false;
		}
		
		var callbackFunc = thisObj.callbackFunc;
		var itemObj = thisObj.item_obj;		
		...
		if (this.price_id != null) {
			$.post('<?php echo U("financial/price");?>', {op_type: 'find', id: this.price_id}, function(data){
				if (data.ds != null) {					
					...
					setSelect2Value($(modalRootObj).find('#data_fprice_line'), d.line);
					// 备注
...
<div class="item-container template-item_container hidden_ctrl">
	...
	<div class="item-footer" style="text-align: center;">
		<a href="javascript:;" style="width: 10%;" class="more" data-more="1">点击加载更多价格项<i class="fa-arrow-down"></i></a>		
	</div>
</div>
...
<script type="text/javascript">
	...
	function ModalCJItem(vobj, objType, objId) {
		...
		$(this.root).find('.more').bind('click', {obj:this}, this.findMoreItem);
	}
						...
						<ul>
							...
							<li <?php echo stripos(C('MENU_CURRENT'), 'financial_team') === false ? '' : 'class="active opened"';?>>
								...
								<ul>
									...
									<li <?php echo stripos(C('MENU_CURRENT'), 'financial_deposit_list') === false ? '' : 'class="active"';?>>
				...
				<div class="col-sm-12">
					<div class="tab-content"  style="border-top: 1px solid #40bbea; margin-top: 0px;">
						<div class="tab-pane active" id="order">
							...
							<div class="col-sm-3">
								<div class="input-group">
									...
									<select class="form-control" id="order_from">
										<option value="-1">不限</option>
										<?php if(!empty($OrderFrom)): if(is_array($OrderFrom)): $i = 0; $__LIST__ = $OrderFrom;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$of): $mod = ($i % 2 );++$i;?><option value="<?php echo ($of["id"]); ?>"><?php echo ($of["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
									</select>
								</div>
							</div>
							<div class="col-sm-3">
							...
							<div class="col-sm-3">
								<div class="input-group">
									...
									<select class="form-control" id="cash_use">
										<option value="-1">不限</option>
										<?php if(!empty($CashUse)): if(is_array($CashUse)): $i = 0; $__LIST__ = $CashUse;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cu): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cu["id"]); ?>"><?php echo ($cu["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
									</select>
								</div>
							</div>
							<div class="col-sm-3">
								<div class="input-group">
									...
									<select class="form-control" id="cash_func">
										<option value="-1">不限</option>
										<?php if(!empty($CashFunc)): if(is_array($CashFunc)): $i = 0; $__LIST__ = $CashFunc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cf): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cf["id"]); ?>"><?php echo ($cf["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
									</select>
								</div>
							</div>
							<div class="col-sm-3">
							...
							<div class="col-sm-3">
								<div class="input-group">
									<span class="input-group-addon info-color text-white cx_border01">核审人账户:</span>
									<input type="text" class="form-control cx_border02" id="review_admin_acct">
								</div>
							</div>
...
<script type="text/javascript">
	...
	function initDataTable() {	
		var vTable = $("#example-1").dataTable({
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": '<?php echo U("financial/deposit");?>',
			//如果加上下面这段内容，则使用post方式传递数据
			"fnServerData": getPageData,
			"aoColumns": [
				{"mData": "id", "aTargets": [0], "fnCreatedCell": replaceContent},
				{"mData": "deposit_obj", "aTargets": [1], "fnCreatedCell": replaceContent},
				{"mData": "depoist_type", "aTargets": [2], "fnCreatedCell": replaceContent},
				{"mData": "request_content", "aTargets": [3], "fnCreatedCell": replaceContent},
				{"mData": "request_admin", "aTargets": [4], "fnCreatedCell": replaceContent},
				{"mData": "request_time", "aTargets": [5], "fnCreatedCell": replaceContent},
				{"mData": "review_type", "aTargets": [6], "fnCreatedCell": replaceContent},
				{"mData": "review_admin", "aTargets": [7], "fnCreatedCell": replaceContent},
				{"mData": "review_time", "aTargets": [8], "fnCreatedCell": replaceContent},
	...
	function getFindData() {		
		var jsonData = [
				...
				{'name':'review_admin_acct', 'value':$('#review_admin_acct').val()},
...
//		aoData.push({'name':'data','value':jsonData});
		console.log(JSON.stringify(aoData));
		$.post('<?php echo U("financial/deposit");?>', aoData, function(data){		
			showLoading(false);
			if (data['result']['errno'] == 0) {
				fnCallback(data);
			} else {
...
//		alert(JSON.stringify(oData['review_complate']));
	...
	function _editFun(aid, bComplate) {
		$.getJSON("<?php echo U('review/request');?>", {op:'review_response', id:aid, complate: bComplate}, function(data){
