						...
						<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
							<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$keyword): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($keyword["id"]); ?>">
										<?php else: ?>
											<tr role="row" class="even" data-id="<?php echo ($keyword["id"]); ?>"><?php endif; ?>
											<td><?php echo ($keyword["id"]); ?></td>
											<td><?php echo ($keyword["type"]); ?></td>
											<td><?php echo ($keyword["page_name"]); ?></td>
											<td><?php echo ($keyword["page"]); ?></td>
...
<script type="text/javascript">
	...
	$('.btn-save').click(function(){
		...
		$.post('<?php echo U("settle/keywords");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.type != null && data.type == 'create') {
					location.href = '<?php echo U("settle/keywords");?>';
				} else {
					if (data.ds != null) {
						var d = data.ds;
	...
	function deleteData() {
		...
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("settle/keywords");?>', {op_type:'delete', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
