					...
					<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
						<table class="table table-bordered table-striped dataTable no-footer" id="table_template" role="grid" aria-describedby="example-2_info">
							...
							<tbody class="middle-align sms-tbody">
								<?php if(is_array($templates)): $i = 0; $__LIST__ = $templates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($t["id"]); ?>">
									<?php else: ?>
...
<script type="text/javascript">
	...
	function deleteTemplate() {
		...
		$.post('<?php echo U("settle/smstemplate");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				$(trObj).remove();
			}
			alert(data.result.message);
