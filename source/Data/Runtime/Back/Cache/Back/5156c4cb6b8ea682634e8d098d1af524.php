...
<?php if(!empty($modal_fcj_deposit)): ?><div class="modal fade custom-width" id="gallery-fcj-deposit-modal" data-backdrop="static">
	<div class="modal-dialog" style="width: 70%;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-white" data-dismiss="modal">关闭</button>
				<button type="button" class="btn btn-info btn_fdeposit_submit">提审预算</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	ModalFCJDeposit.prototype.showModal = function(vobj, ids, reviewId, confirmCallBack){
		this.obj = vobj;
		this.ids = ids;
	...
	ModalFCJDeposit.prototype.fillItem = function() {
		...
		if (this.review_id != null && this.ids != 'undefined' && this.ids != '') {
		...
		$.post('<?php echo U("financial/team");?>', jsonData, function(data){
			if (data.ds != null) {			
				...
				for (var i=0; i < data.ds.length; i ++) {							
					var d = data.ds['i'];
					if (d.obj_data != null) {
						if (d.obj_data.payee != null) {
							$('#data_fdeposit_payee').val(d.obj_data.payee);
						}
						if (d.obj_data.bank_card != null) {
							$('#data_fdeposit_bank_card').val(d.obj_data.bank_card);
						}
						if (d.obj_data.bank_address != null) {
							$('#data_fdeposit_bank_address').val(d.obj_data.bank_address);
						}
					}
					
					if (d.review_data != null) {
						$('#data_fdeposit_beizhu').val(d.review_data.beizhu);
					}
				}			
			} else {
	...
	ModalFCJDeposit.prototype.confirmItem = function(e){
		...
		var callbackFunc = e.data.obj.callbackFunc;
		var itemObj = e.data.obj.item_obj;		
		$.post('<?php echo U("financial/team");?>', param, function(data){
			if (data.result != null) {
				if (data.extra_result != null) {
					if (data.result.errno != 0 && data.extra_result.errno != 0){
						toastr.error(data.result.message + '<br />' + data.extra_result.message);
...
<div class="row">	
	<div class="col-md-1">						
		<div class="form-group">
		    ...
		    <input id="sms_inf" name="sms_inf" data-id="<?php echo ($sms_set["id"]); ?>" type="checkbox" class="iswitch iswitch-secondary">
		</div>						
	</div>
</div>
			<!-- Main Footer -->
...
<script type="text/javascript">
	$('#sms_inf').change(function(){
		...
		$.post('<?php echo U("settle/especial");?>', jsonData, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
	...
	function initSmsSet() {
		if ('<?php echo ($sms_set["value"]["forbidden"]); ?>' != '') {
			$('#sms_inf').prop('checked', '<?php echo ($sms_set["value"]["forbidden"]); ?>'=='0' ? true : false).trigger();	
		}		
	}
