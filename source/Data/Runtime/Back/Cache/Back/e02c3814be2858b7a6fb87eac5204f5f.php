						...
						<div class="create-write" data-id="">
							...
							<div class="row">
								...
								<div class="col-md-2">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">驾龄</i>
										</div>
										<input type="number" class="form-control data_year" maxlength="1" />
									</div>
								</div>
								<div class="col-md-4">
									<div class="input-group">
										...
										<input type="text" class="form-control data_id_card" maxlength="18" />
									</div>
								</div>
								<div class="col-md-4">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">驾照</i>
										</div>
										<input type="text" class="form-control data_driving_license" maxlength="18" />
									</div>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">联系电话</i>
										</div>
										<input type="number" class="form-control data_mob" maxlength="11" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
				...
				<div class="panel-body">						
					...
					<div class="issuing-list">
						...
						<div class="issuing-list-search">
							<div class="input-group">
								<input type="text" class="form-control" placeholder="搜索内容可以是编号、所属线路、姓名、电话、身份证、驾照">
							...
							<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
								<thead>
									<tr role="row" class="head">
										...
										<th class="sorting" rowspan="1" colspan="1" style="width: 50px;">驾龄</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 80px;">电话</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 100px;">证件</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 100px;">驾照</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">所属线路</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 100px;">结算方式</th>
								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ds): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($ds["id"]); ?>">
										<?php else: ?>
											...
											<td><?php echo ($ds["years"]); ?></td>
											<td><?php echo ($ds["mob"]); ?></td>
											<td><?php echo ($ds["id_card"]); ?></td>
											<td><?php echo ($ds["driving_license"]); ?></td>
...
<script type="text/javascript">
	...
	$('.btn-save').click(function(){
		var jsonData = {
			...
			driving_license: $('.data_driving_license').val(),
			mob: $('.data_mob').val(),
		...
		$.post('<?php echo U("financial/source");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.op != null && data.op == 'create') {
					location.href = '<?php echo U("financial/source");?>/src/driver';
				} else {
					if (data.ds != null) {
						var d = data.ds;
						var trObj = $('tr[data-id="'+d.id+'"]');
						$(trObj).find('td:eq(1)').html(d.name);
						$(trObj).find('td:eq(2)').html(d.years);
						$(trObj).find('td:eq(3)').html(d.mob);
						$(trObj).find('td:eq(4)').html(d.id_card);
						$(trObj).find('td:eq(5)').html(d.driving_license);
	...
	var itemObjs = {};
	function itemData(){
		var driverId = $(this).closest('tr').attr('data-id');
		var itemObj = itemObjs['item_'+driverId];
		if (itemObj == null) {
			itemObj = new ModalCJItem(this, 'driver', driverId);
			itemObjs['item_'+driverId] = itemObj;
		}	
		itemObj.showItems();
	}
	...
	function editData(){
		...
		$.post('<?php echo U("financial/source");?>', {op_type:'find', src:'driver', id:did}, function(data){
			if (data.ds != null) {
				var d = data.ds;
				...
				$('.data_mob').val(d.mob);
				$('.data_driving_license').val(d.driving_license);
	...
	function deleteData() {
		...
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("financial/source");?>', {op_type:'delete', src: 'driver', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
					$(trObj).remove();
	...
	function findPageData(p) {			
		...
		if (searchval != '') {
			jsonData['cdsstr'] = 'id|=|'+searchval+'|OR|line|LIKE|%'+searchval+'%|OR|name|LIKE|%'+searchval+'%|OR|mob|LIKE|%'+searchval+'%|OR|id_card|LIKE|%'+searchval+'%|OR|driving_license|LIKE|%'+searchval+'%|OR';
		}
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">';
							...
							vhtml +='	<td>'+d.years+'</td>';
							vhtml +='	<td>'+d.mob+'</td>';
							vhtml +='	<td>'+d.id_card+'</td>';
							vhtml +='	<td>'+d.driving_license+'</td>';
