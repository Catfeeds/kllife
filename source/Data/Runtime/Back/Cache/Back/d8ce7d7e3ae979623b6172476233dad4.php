						...
						<div class="create-write" data-id="">
							...
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">景点名称</i>
										</div>
								...
								<div class="col-md-3">
									<div class="input-group">
										...
										<input type="number" class="form-control data_fax" maxlength="11" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">省份</i>
										</div>
										<input type="text" class="form-control data_province" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
							...
							<div class="row">
								...
								<div class="col-md-6">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">景点地址</i>
										</div>
				...
				<div class="panel-body">						
					...
					<div class="issuing-list">
						...
						<div class="issuing-list-search">
							<div class="input-group">
								<input type="text" class="form-control" placeholder="搜索内容可以是编号、所属线路、景点名称、地址、省份、城市、联系人、联系人电话、传真">
							...
							<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
								<thead>
									<tr role="row" class="head">
										...
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">景点</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">地点</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 120px;">联系方式</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">所属线路</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">结算方式</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">备注</th>
								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ds): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($ds["id"]); ?>">
										<?php else: ?>
											...
											<td>省份:<?php echo ($ds["province"]); ?><br />城市:<?php echo ($ds["city"]); ?><br />地址:<?php echo ($ds["address"]); ?></td>
											<td>联系人:<?php echo ($ds["contacts"]); ?><br />电话:<?php echo ($ds["contacts_mob"]); ?><br />传真:<?php echo ($ds["fax"]); ?></td>
...
<script type="text/javascript">
		...
		$.post('<?php echo U("financial/source");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.op != null && data.op == 'create') {
					location.href = '<?php echo U("financial/source");?>/src/view';
				} else {
					if (data.ds != null) {
						var d = data.ds;
						var trObj = $('tr[data-id="'+d.id+'"]');
						$(trObj).find('td:eq(1)').html(d.name);
						$(trObj).find('td:eq(2)').html('省份:'+d.province+'<br />城市:'+d.city+'<br />地址:'+d.address);
						$(trObj).find('td:eq(3)').html('联系人:'+d.contacts+'<br />电话:'+d.contacts_mob+'<br />传真:'+d.fax);
						...
						$(trObj).find('td:eq(6)').html(d.beizhu);
					}
					clearInfo();
	...
	var itemObjs = {};
	function itemData(){
		var viewId = $(this).closest('tr').attr('data-id');
		var itemObj = itemObjs['item_'+viewId];
		if (itemObj == null) {
			itemObj = new ModalCJItem(this, 'view', viewId);
			itemObjs['item_'+viewId] = itemObj;
		}	
		itemObj.showItems();
	}
	...
	function editData(){
		...
		$.post('<?php echo U("financial/source");?>', {op_type:'find', src:'view', id:did}, function(data){
			if (data.ds != null) {
				var d = data.ds;
				$('.create-write').attr('data-id', d.id);
				$('.data_name').val(d.name);
				$('.data_province').val(d.province);
	...
	function deleteData() {
		...
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("financial/source");?>', {op_type:'delete', src: 'view', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
					$(trObj).remove();
	...
	function findPageData(p) {			
		...
		if (searchval != '') {
			jsonData['cdsstr'] = 'id|=|'+searchval+'|OR|line|LIKE|%'+searchval+'%|OR|name|LIKE|%'+searchval+'%|OR|province|LIKE|%'+searchval+'%|OR|city|LIKE|%'+searchval+'%|OR|fax|LIKE|%'+searchval+'%|OR|contacts|LIKE|%'+searchval+'%|OR|contacts_mob|LIKE|%'+searchval+'%|OR|address|LIKE|%'+searchval+'%|OR';
		}
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">';
							vhtml +='	<td>'+d.id+'</td>';
							vhtml +='	<td>'+d.name+'</td>';
							vhtml +='	<td>省份:'+d.province+'<br />城市:'+d.city+'<br />地址:'+d.address+'</td>';
							vhtml +='	<td>联系人:'+d.contacts+'<br />电话:'+d.contacts_mob+'<br />传真:'+d.fax+'</td>';
							vhtml +='	<td>'+d.line_show+'</td>';
							vhtml +='	<td>收款人:'+d.payee+'<br />卡号:'+d.bank_card+'<br />开户行:'+d.bank_address+'<br />结算方式:'+d.pay_type_show+'</td>';
							vhtml +='	<td>'+d.beizhu+'</td>';
							vhtml +='	<td>';
