			...
			<div class="panel panel-default">
				...
				<div class="panel-body row">
					<div id="dingdan_main" class="col-sm-12" style="margin-left: 5%;">
						<div class="row">							
							...
							<div class="row form-group">
								...
								<span class="col-sm-8" style="padding-left: 0; display: <?php echo empty($team)?'none;':'display;';?>"><?php echo ($team["title"]); echo ($team["meet_time"]); ?></span>
							</div>
							...
							<div class="row dingdan_bottom">
								...
								<input id='contact_name' type="text" value="<?php echo ($team["linkman"]); ?>" style="height: 32px;" class="dingdan_bianji col-sm-3">
							</div>
							
							<div class="row dingdan_bottom">
								<p class="col-sm-1">联系人电话：</p>
								<input id="contact_tel" type="text" value="<?php echo ($team["tel"]); ?>" maxlength="11" style="height: 32px;" class="dingdan_bianji col-sm-3">
							</div>
							...
							<div class="row dingdan_bottom">
								...
								<?php if(is_array($memberType)): $i = 0; $__LIST__ = $memberType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$mt): $mod = ($i % 2 );++$i;?><span class="col-sm-1 text-left" style="padding-left: 0;">									
										<input id="<?php echo ($mt["type_key"]); ?>" class="dingdan_bianji" style="width: 30%" maxlength="3" type="text" value="0"/>位<?php echo ($mt["type_desc"]); ?>
					...
					<div class="col-sm-12" style="margin-left: 5%; margin-top: 2%;">
						...
						<?php if(grant_create): ?><button id="create_order" type="button" class="btn btn-primary" style="margin-left: 1%;">新增订单</button><?php endif; ?>
...
//		}
		...
		$.post('<?php echo U("team/order");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				alert(data.result.message);
	...
	function changeSumCash() {
		...
		for (var i=0; i<$(types).length; i++) {
			...
			if (idstr.indexOf('child') != -1) {
				sumCash += (parseFloat('<?php echo ($team["price_child"]); ?>') * memberCount);	
			} else {
				sumCash += (parseFloat('<?php echo ($team["price_adult"]); ?>') * memberCount);
			}
		}
