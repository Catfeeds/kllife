										...
										<tbody class="middle-align coupon-tbody">
											<?php if(is_array($coupons)): $i = 0; $__LIST__ = $coupons;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($c["id"]); ?>">
												<?php else: ?>
													...
													<td><?php echo ($c["user"]["show_name"]); ?></td>
													<td><?php echo ($c["type_data"]["type_desc"]); ?></td>
													<td><?php echo ($c["money"]); ?></td>
													<td><?php if($c['overlying'] > 0): ?>可叠加<?php else: ?>不可叠加<?php endif; ?></td>
													<td><?php if($c['forever'] > 0): ?>永久<?php else: ?>非永久<?php endif; ?></td>
													<td><?php echo ($c["send_acct_data"]["show_name"]); ?></td>
													<td><?php echo ($c["send_time"]); ?></td>
													<td><?php echo ($c["valid_time"]); ?></td>
													<td><?php echo ($c["invalid_time"]); ?></td>
													<td><?php echo ($c["use_time"]); ?></td>
													<td><?php echo ($c["order_id_data"]["order_sn"]); ?></td>
													<td><?php if($c['invalid'] > 0): ?>无效<?php else: ?>有效<?php endif; ?></td>
													<td>
														<?php if($c['invalid'] == 0): ?><a href="#" class="btn btn-danger btn-sm btn-icon icon-left btn_delete_coupon">删除</a><?php endif; ?>
													</td>
												</tr><?php endforeach; endif; else: echo "" ;endif; ?>
										</tbody>
									</table>
									<div class="list-page coupon-list-page" data-page-index="1" data-page-count="<?php echo ($page_count); ?>"></div>
								</div> <!--结束优惠券表格-->
...
<script type="text/javascript">	
	...
	function findUser(p, forceCreatePage) {		
		...
		$.post('<?php echo U("user/coupon");?>', jsonData, function(data){
	...
	// 构建优惠券分页
	constructionPage($('.coupon-list-page'), 1, '<?php echo ($page_count); ?>', findCoupon, true);
