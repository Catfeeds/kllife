...
//						});
		...
		<div class="col-sm-12">
			<div class="panel panel-default">
				...
				<!--在此地编辑内容-->
					<!--创建优惠券-->
					<div class="create">
						<div class="create-write" data-id="<?php echo ($leader["id"]); ?>">
							...
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										<p>领队详情页头像</p>
										<img class="xiuxiu_upload data_face_image" data-set="face_image" width="100%" src="<?php echo ($leader["face_image"]); ?>" alt="详情页领队展示图片"/>
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<p>导游证图片</p>
										<img class="xiuxiu_upload data_leader_certificate_image" data-set="leader_certificate_image" width="100%" src="<?php echo ($leader["leader_certificate_image"]); ?>" alt="导游证图片"/>
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<p>身份证正面照</p>
										<img class="xiuxiu_upload data_id_card_image_front" width="100%" data-set="id_card_image_front" src="<?php echo ($leader["id_card_image_front"]); ?>" alt="身份证正面图片"/>
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<p>身份证反面照</p>
										<img class="xiuxiu_upload data_id_card_image_behind" width="100%" data-set="id_card_image_behind" src="<?php echo ($leader["id_card_image_behind"]); ?>" alt="身份证反面图片"/>
									</div>
								</div>
							</div>
							<div class="row">
					...
					<section class="profile-env">
						...
						<section class="user-timeline-stories">
							<?php if(is_array($questions)): $i = 0; $__LIST__ = $questions;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$quest): $mod = ($i % 2 );++$i;?><hr class="separator-line"/>
								...
								<article class="timeline-story" data-id="<?php echo ($quest["id"]); ?>" data-leader-id="{quest.leader_id}">
									<i class="fa-paper-plane-empty block-icon"></i>
									<!-- User info -->
									<header>							
										<!-- Story Content Wrapped inside Paragraph -->
	...
	<script type="text/javascript">
		// 初始化翻页
		constructionPage('.question_page', 1, '<?php echo ($question_page_count); ?>', changeQuestionPage, true);
		// 添加问题HTML
...
//			console.log('container:'+container+',before'+before+',ds:'+JSON.stringify(ds));										
		...
		function changeQuestionPage(p) {
			var leaderId = '<?php echo ($leader["id"]); ?>';
			var jsonData = {
				op_type: 'list_question',
				leader_id: leaderId,
				page: p-1,
			}
			$.post('<?php echo U("financial/leader");?>', jsonData, function(data){
				if (data.result.errno == 0) {					
...
//			var content = $(rootObj).find('.answer_reply_textarea').data('wysihtml5').editor.getValue();
		...
		function saveContent(jsonData) {			
			...
			$.ajax({
				url: '<?php echo U("financial/leader");?>',
				type: 'POST',
				async: false,
				data: jsonData,
				dataType: 'json',
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<div class="row photo_list">
						<?php if(is_array($pictures)): $i = 0; $__LIST__ = $pictures;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pic): $mod = ($i % 2 );++$i;?><div class="col-xs-3 photo_item" data-id="<?php echo ($pic["id"]); ?>">
								<img class="img-responsive" src="<?php echo ($pic["path"]); ?>" alt="">
								<img class="photo_remove" src="/source/Static/back/images/error.png" alt="">
	...
	<script type="text/javascript">
		// 初始化翻页
		constructionPage('.picture_page', 1, '<?php echo ($picture_page_count); ?>', changePicturePage, true);
		// 作品添加成功
				...
				if (data.ds != null) {
					for (var i=0; i < data.ds.length; i++) {
						...
						var vhtml = '<div class="col-xs-3 photo_item" data-id="'+d.id+'">';
							...
							vhtml +='	<img class="photo_remove" src="/source/Static/back/images/error.png" alt="">';
							vhtml +='</div>';
		...
		$('.btn_append_leader_picture').click(function(){
			...
			showModalLeaderUploadDialog($('.photo_list'), '<?php echo ($leader["id"]); ?>');
		});
...
<script type="text/javascript">	
	...
	$('.xiuxiu_upload').click(function(){
		...
		if (myxx == null) {
			var url = 'http://kllife.com<?php echo U("financial/leader");?>';
