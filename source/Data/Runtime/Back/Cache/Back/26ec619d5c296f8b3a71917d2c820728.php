					...
					<div id="dingdan_main" class="col-sm-12" style="margin-left: 5%;">
						<div class="row">
							<div class="form-group row">
								<p id='err_msg' class="col-sm-3" style="color:#e4393c;"><?php echo ($aa); ?></p>
							</div>
							<div class="form-group row">
								<p class="col-sm-1">线路名称：</p>	
								<span class="col-sm-8" style="padding-left: 0; display: <?php echo empty($order)?'none;':'display;';?>"><?php echo ($order["lineid_title"]); ?></span>
								<div class="input-group col-sm-5" style="float: left; display: <?php echo empty($order)?'display;':'none;';?>">
									<script type="text/javascript">
										jQuery(document).ready(function($)
										{
											$("#line_article").select2({
												...
												formatSelection: function(data) { 
													// 线路批次													
													$.post('<?php echo U("order/activityFind");?>', {aid:data.id}, function(resp){
														var html = '';
													...
													// 订单编号											
													$.post('<?php echo U("order/ordersnFind");?>', {lineid:data.id}, function(resp){
														var html = '';
														if (resp['result']['errno'] == 0 ){
															for (var i = 0; i < resp['data'].length; i ++) {																
																var ds = resp['data'][i];
							...
							<div class="form-group row">
								...
								<span class="col-sm-5" style="padding-left: 0; display: <?php echo empty($order)?'none;':'display;';?>"><?php echo ($order["hdid_show_text"]); ?></span>
								<div class="input-group col-sm-5" style="float: left; display: <?php echo empty($order)?'display;':'none;';?>">
									<script type="text/javascript">
											jQuery(document).ready(function($)
											{
												...
												$(s2exp).on('change', function(){
													...
													// 订单编号											
													$.post('<?php echo U("order/ordersnFind");?>', {lineid:lineId,hdid:activityId}, function(resp){
														var html = '';
														if (resp['result']['errno'] == 0 ){
															for (var i = 0; i < resp['data'].length; i ++) {																
																var ds = resp['data'][i];
							...
							<div class="form-group row">
								...
								<span class="col-sm-5" style="padding-left: 0; display: <?php echo empty($order)?'none;':'display;';?>"><?php echo ($order["order_sn"]); ?></span>
								<div class="input-group col-sm-5" style="float: left; display: <?php echo empty($order)?'display;':'none;';?>">
									<script type="text/javascript">
											jQuery(document).ready(function($)
											{
							...
							<div class="form-group row">
								...
								<span id="order_state" class="col-sm-5" style="padding-left: 0;"><?php echo ($order["zhuangtai_data"]["type_desc"]); ?></span>
							</div>
							<div class="form-group row">
								<p class="col-sm-1">订单总金额：</p>								
								<span id="order_sum_cash" class="col-sm-5" style="padding-left: 0;"><?php echo ($order["need_pay_money"]); ?></span>
							</div>
							<div class="form-group row">
								<p class="col-sm-1">订单已付金额：</p>								
								<span id="order_pay_cash" class="col-sm-5" style="padding-left: 0;"><?php echo ($order["pay_cash"]); ?></span>
							</div>
							<div class="form-group row">
								<p class="col-sm-1">用户名：</p>								
								<span id="username" class="col-sm-5" style="padding-left: 0;"><?php echo ($order["userid_data"]["show_name"]); ?></span>
							</div>
							<div class="form-group row">
								<p class="col-sm-1">联系人：</p>								
								<span id="contact" class="col-sm-5" style="padding-left: 0;"><?php echo ($order["names"]); ?></span>
							</div>
							<div class="form-group row">
								<p class="col-sm-1">联系人电话：</p>								
								<span id="mobile" class="col-sm-5" style="padding-left: 0;"><?php echo ($order["mob"]); ?></span>
							</div>
							<div class="form-group row">
...
//											});
									...
									<select id="cash_func" class="form-control">
										<?php if(is_array($CashFunc)): $i = 0; $__LIST__ = $CashFunc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cf): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cf["id"]); ?>"><?php echo ($cf["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
							<div class="form-group row">
								<div class="row" style="margin-left: 0px;">
									...
									<div class="col-sm-3" style="border-color: #40bbea; padding-left: 0; padding-right: 0;">
										...
										<select name="cash_use" class="form-control">
											<?php if(is_array($CashUse)): $i = 0; $__LIST__ = $CashUse;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cu): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cu["id"]); ?>"><?php echo ($cu["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
									</div>
							...
							<div class="form-group row">
								...
								<div class="input-group col-sm-3" style="border-color: #40bbea;">
									...
									<select id="pay_type" class="form-control">
										<?php if(is_array($MenuItem)): $i = 0; $__LIST__ = $MenuItem;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$mi): $mod = ($i % 2 );++$i;?><option value="<?php echo ($mi["id"]); ?>"><?php echo ($mi["item_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										<!--<?php if(is_array($PayType)): $i = 0; $__LIST__ = $PayType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pt): $mod = ($i % 2 );++$i;?><option value="<?php echo ($pt["id"]); ?>"><?php echo ($pt["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>-->
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="input-group col-sm-3" style="border-color: #40bbea;">
									...
									<select id="pay_channel" class="form-control">
										<!--<?php if(is_array($PayChannel)): $i = 0; $__LIST__ = $PayChannel;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pc): $mod = ($i % 2 );++$i;?><option value="<?php echo ($pc["id"]); ?>"><?php echo ($pc["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>-->
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="input-group col-sm-3" style="border-color: #40bbea;">
									...
									<select id="pay_bank" class="form-control">
										<!--<?php if(is_array($PayBank)): $i = 0; $__LIST__ = $PayBank;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pb): $mod = ($i % 2 );++$i;?><option value="<?php echo ($pb["id"]); ?>"><?php echo ($pb["type_desc"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>-->
									</select>
								</div>
							</div>
							<div class="form-group row">
							...
							<div class="row">
								<?php if($grant_submit_review == true): ?><button id="review_request" type="button" class="btn btn-warning" style="margin-left: 1%;">提交审核</button><?php endif; ?>
								<?php if($grant_response_review == true): ?><button id="review_response" type="button" class="btn btn-warning" style="margin-left: 1%;">一键审核</button><?php endif; ?>
								<?php if(!empty($order)): ?><button id="back_list" type="button" class="btn btn-secondary" style="margin-left: 1%;">返回列表</button><?php endif; ?>
							</div>
						</div>
					</div>
...
//		$(divRootCopy).find('[name="cash_use"]').change(changeCashUsePrice);
	...
	function createReview(flag) {
		...
		showLoading(true);		
		$.post('<?php echo U("review/request");?>', jsonData, function(data){
			showLoading(false);
			if (data.result.errno == 0) {
				alert('订单审核申请成功');
				location.href='<?php echo U("review/list");?>';
			} else {
				alert(data.result.message);
//				$('#err_msg').html(data.result.message);
			}
