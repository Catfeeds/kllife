...
//						});
						...
						<div class="create-write">
							<div class="row">			
								...
								<div class="col-md-7" style="height: 280px; overflow: auto;">
									<div class="row">
										<?php if(is_array($destination)): $i = 0; $__LIST__ = $destination;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dest): $mod = ($i % 2 );++$i;?><button class="btn btn-info btn_dest" data-key="<?php echo ($dest["type_key"]); ?>"><?php echo ($dest["type_desc"]); ?></button><?php endforeach; endif; else: echo "" ;endif; ?>
									</div>
								</div>							
							</div>
		...
		<div class="col-sm-12">
			<div class="panel panel-default">
				...
				<div class="panel-body" style="padding: 20px 20px; width: 100%; overflow: auto;overflow-x: scroll;">	
					...
					<table id="table_total" class="table">
						<thead>
							<tr>
								...
								<th class="font-table-th" style="width: 130px;">指定日期订单数</th>
								<th class="font-table-th" style="width: 130px;">指定日期人数</th>
					...
					<table id="table_order" class="table" style="width:200%;max-width:200%;">
						<thead>
							<tr>
								<th class="font-table-th" style="width: 200px;">线路批次</th>
							</tr>
						</thead>
						<tbody></tbody>
					</table>
	...
	<script type="text/javascript">	
		...
		$('.btn_dest').click(function(){
			...
			$.post('<?php echo U("statistics/line");?>', jsonData, function(data){
				showLoading(false);
				if (data.result.errno == 0) {
					resetStatsObj();
					if (data.ds != null) {
		...
		function getStatsBatchData() {
			...
			if (vtype == 0) {
				...
				batch['show_text'] = batch['start_time'].split(' ')[0];
				return batch;
			} else if (vtype == 1) {
				...
				var batch = $.parseJSON(batchstr);
				batch['show_text'] = UnixtimeToDatetime(batch['startdate'], true);
				return batch;
			} else if (vtype == 2) {
		...
		$('.btn-start-stats').click(function(){			
			...
			$.post('<?php echo U("statistics/line");?>', jsonData, function(data){
				...
				if (data.result.errno == 0) {
					if (data.ds != null) {
						// 滚动到结果
       					$('body').animate({scrollTop:$('#table_total').offset().top}, 500);
						$('#table_total').find('tbody').children().remove();
						$('#table_order').find('thead tr').children().remove();
						$('#table_order').find('thead tr').append('<th class="font-table-th" style="width: 200px;">线路批次</th>');
						...
						for (var i=0; i<data.ds.length; i++) {
							...
							var vhtml = '<tr>'+
										...
										'</tr>';
							$('#table_total').find('tbody').append(vhtml);
							...
							if (d.stats_channels != null) {								
								...
								// 表数据
								var trColor = i % 2 == 0 ? '#D6C4AD' : 'burlywood';
								var orderHtml = '<tr style="background-color: '+trColor+';"><td>'+d.title+'</td>';
								for (k in d.stats_channels) {
									var channel = d.stats_channels[k];							
									orderHtml += '<td>订单:'+channel.order_count+'<br />人数:'+channel.member_count+'</td>';
								}
								orderHtml += '</tr>';
