						...
						<div class="create-write" data-id="">
							...
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">车队名称</i>
										</div>
								...
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">车牌号</i>
										</div>
										<input type="text" class="form-control data_bus_no" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">车龄(年)</i>
										</div>
										<input type="number" class="form-control data_years" maxlength="2" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">品牌</i>
										</div>
										<input type="text" class="form-control data_bus_logo" />
									</div>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">车间距(米)</i>
										</div>
										<input type="number" class="form-control data_distance" maxlength="2" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">座位数</i>
										</div>
										<input type="text" class="form-control data_seat" />
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										<div class="input-group-addon">
											<i style="font-style: normal;">动力类型</i>
										</div>
										<script type="text/javascript">
											$(document).ready(function(){
												bindTypeDataDropMenu($('.data_dynamic_type'), 'cj_bus_dynamic', '选择大巴动力类型');
											});
										</script>
										<input type="text" class="form-control data_dynamic_type">
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
							...
							<div class="row">
								<div class="col-md-12">
									<label class="checkbox-inline" style="margin-left: 20px!important;"><input type="checkbox" class="checkbox data_park">含停车费</label>
									<label class="checkbox-inline" style="margin-left: 50px!important;"><input type="checkbox" class="checkbox data_oil">含油费</label>
									<label class="checkbox-inline" style="margin-left: 50px!important;"><input type="checkbox" class="checkbox data_road_toll">含养路费</label>
									<label class="checkbox-inline" style="margin-left: 50px!important;"><input type="checkbox" class="checkbox data_luggage">行李舱</label>
									<label class="checkbox-inline" style="margin-left: 50px!important;"><input type="checkbox" class="checkbox data_air_conditioner">空调</label>
									<label class="checkbox-inline" style="margin-left: 50px!important;"><input type="checkbox" class="checkbox data_wifi">WIFI</label>
									<label class="checkbox-inline" style="margin-left: 50px!important;"><input type="checkbox" class="checkbox data_double_driver">双驾</label>
								</div>
							</div>
				...
				<div class="panel-body">						
					...
					<div class="issuing-list">
						...
						<div class="issuing-list-search">
							<div class="input-group">
								<input type="text" class="form-control" placeholder="搜索内容可以是编号、所属线路、车牌号、品牌、车间距、动力类型、联系人、联系人电话">
							...
							<table class="table table-bordered table-striped dataTable no-footer" id="table_list" role="grid" aria-describedby="example-2_info">
								<thead>
									<tr role="row" class="head">
										...
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">车队</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">硬件配置</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">软件配置</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">所属线路</th>
										<th class="sorting" rowspan="1" colspan="1" style="width: 150px;">结算信息</th>
								...
								<tbody class="middle-align list-tbody">
									<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ds): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($ds["id"]); ?>">
										<?php else: ?>
											<tr role="row" class="even" data-id="<?php echo ($ds["id"]); ?>"><?php endif; ?>
											<?php $dynamicType = json_decode($ds['dynamic_type'], true); $conds = ''; if (!empty($ds['seat'])) { $conds = '<span style="color:red;">'.$ds['seat'].'座'.$ds['bus_logo'].'</span>'; } if (!empty($ds['is_park'])) { $conds .= empty($conds) ? '含停车费' : ',含停车费'; } if (!empty($ds['is_oil'])) { $conds .= empty($conds) ? '含油费' : ',含油费'; } if (!empty($ds['is_road_toll'])) { $conds .= empty($conds) ? '含养路费' : ',含养路费'; } if (!empty($ds['luggage'])) { $conds .= empty($conds) ? '有行李箱' : ',有行李箱'; } if (!empty($ds['air_conditioner'])) { $conds .= empty($conds) ? '有空调' : ',有空调'; } if (!empty($ds['wifi'])) { $conds .= empty($conds) ? '有WIFI' : ',有WIFI'; } if (!empty($ds['double_driver'])) { $conds .= empty($conds) ? '有双驾' : ',有双驾'; } ?>
											<td><?php echo ($ds["id"]); ?></td>
											<td><?php echo ($ds["name"]); ?></td>
											<td>品牌:<?php echo ($ds["bus_logo"]); ?><br />车牌号:<?php echo ($ds["bus_no"]); ?><br />车距:<?php echo ($ds["distance"]); ?><br />动力类型:<?php echo ($dynamicType["type_desc"]); ?><br />车龄:<?php echo ($ds["years"]); ?></td>
											<td><?php echo ($conds); ?></td>
											<td><?php echo ($ds["line_show"]); ?></td>
											<td>收款人:<?php echo ($ds["payee"]); ?><br />卡号:<?php echo ($ds["bank_card"]); ?><br />开户行:<?php echo ($ds["bank_address"]); ?><br />结算方式:<?php echo ($ds["pay_type_show"]); ?></td>
											<td><?php echo ($ds["contacts"]); ?><br><?php echo ($ds["contacts_mob"]); ?></td>
											<td>
...
<script type="text/javascript">
	...
	function clearInfo() {
		...
		$('.data_bus_logo').val('');
		$('.data_distance').val('');
		setSelect2Value($('.data_dynamic_type'), null);
		...
		$('.data_park').prop('checked', false).trigger('change');
		$('.data_oil').prop('checked', false).trigger('change');
		$('.data_road_toll').prop('checked', false).trigger('change');
		$('.data_luggage').prop('checked', false).trigger('change');
		$('.data_air_conditioner').prop('checked', false).trigger('change');
		$('.data_wifi').prop('checked', false).trigger('change');
		$('.data_double_driver').prop('checked', false).trigger('change');
	}	
	...
	$('.btn-save').click(function(){
		var jsonData = {
			...
			bus_no: $('.data_bus_no').val(),
			years: $('.data_years').val(),
			bus_logo: $('.data_bus_logo').val(),
			distance: $('.data_distance').val(),
			dynamic_type: getSelect2Value($('.data_dynamic_type')),
			contacts: $('.data_contacts').val(),
			contacts_mob: $('.data_contacts_mob').val(),
			seat: $('.data_seat').val(),
			is_park: $('.data_park').prop('checked') ? '1' : '0',
			is_oil: $('.data_oil').prop('checked') ? '1' : '0',
			is_road_toll: $('.data_road_toll').prop('checked') ? '1' : '0',
			luggage: $('.data_luggage').prop('checked') ? '1' : '0',
			air_conditioner: $('.data_air_conditioner').prop('checked') ? '1' : '0',
			wifi: $('.data_wifi').prop('checked') ? '1' : '0',
			double_driver: $('.data_double_driver').prop('checked') ? '1' : '0',
		...
		$.post('<?php echo U("financial/source");?>', jsonData, function(data){
			if (data.result.errno == 0) {
				if (data.op != null && data.op == 'create') {
					location.href = '<?php echo U("financial/source");?>/src/bus';
				} else {
					if (data.ds != null) {
						var d = data.ds;
						...
						var dynamicType = '';
						if (d.dynamic_type != null && d.dynamic_type != '') {
							dynamicType = $.parseJSON(d.dynamic_type).type_desc;
						}
						$(trObj).find('td:eq(2)').html('品牌:'+d.bus_logo+'<br />车牌号:'+d.bus_no+'<br />车距:'+d.distance+'<br />动力类型:'+dynamicType+'<br />车龄:'+d.years);
						
						var conds = '';
						if (d.seat > 0) { conds = '<span style="color:red;">'+d.seat+'座'+d.bus_logo+'</span>'; }
						if (d.is_park > 0) { conds += conds == '' ? '含停车费' : ',含停车费'; }
						if (d.is_oil > 0) { conds +=  conds == '' ? '含油费' : ',含油费'; }
						if (d.is_road_toll > 0) { conds +=  conds == '' ? '含养路费' : ',含养路费'; }
						if (d.luggage > 0) { conds +=  conds == '' ? '有行李箱' : ',有行李箱'; }
						if (d.air_conditioner > 0) { conds +=  conds == '' ? '有空调' : ',有空调'; }
						if (d.wifi > 0) { conds +=  conds == '' ? '有WIFI' : ',有WIFI'; }
						if (d.double_driver > 0) { conds +=  conds == '' ? '有双驾' : ',有双驾'; }
						$(trObj).find('td:eq(3)').html(conds);	
						$(trObj).find('td:eq(4)').html(d.line_show);							
						$(trObj).find('td:eq(5)').html('收款人:'+d.payee+'<br />卡号:'+d.bank_card+'<br />开户行:'+d.bank_address+'<br />结算方式:'+d.pay_type_show);
						$(trObj).find('td:eq(6)').html(d.contacts+'<br />'+d.contacts_mob);
					}
					clearInfo();
	...
	var itemObjs = {};
	function itemData(){
		var objId = $(this).closest('tr').attr('data-id');
		var itemObj = itemObjs['item_'+objId];
		if (itemObj == null) {
			itemObj = new ModalCJItem(this, 'bus', objId);
			itemObjs['item_'+objId] = itemObj;
			console.log('new item_'+objId);
		} else {
			console.log('item_'+objId);	
		}
		itemObj.showItems();
	}
	...
	function editData(){
		...
		$.post('<?php echo U("financial/source");?>', {op_type:'find', src:'bus', id:did}, function(data){
			if (data.ds != null) {
				var d = data.ds;
				$('.create-write').attr('data-id', d.id);
				$('.data_name').val(d.name);
				$('.data_bus_no').val(d.bus_no);
				$('.data_years').val(d.years);
				$('.data_bus_logo').val(d.bus_logo);
				$('.data_distance').val(d.distance);
				setSelect2Value($('.data_dynamic_type'), d.dynamic_type);
				setSelect2Value($('.data_line'), d.line);
				$('.data_contacts').val(d.contacts);
				$('.data_contacts_mob').val(d.contacts_mob);
				$('.data_seat').val(d.seat);
				$('.data_park').prop('checked', d.is_park == 1 ? true : false).trigger('change');
				$('.data_oil').prop('checked', d.is_oil == 1 ? true : false).trigger('change');
				$('.data_road_toll').prop('checked', d.is_road_toll == 1 ? true : false).trigger('change');
				$('.data_luggage').prop('checked', d.luggage == 1 ? true : false).trigger('change');
				$('.data_air_conditioner').prop('checked', d.air_conditioner == 1 ? true : false).trigger('change');
				$('.data_wifi').prop('checked', d.wifi == 1 ? true : false).trigger('change');
				$('.data_double_driver').prop('checked', d.double_driver == 1 ? true : false).trigger('change');
	...
	function deleteData() {
		...
		var trObj = $(this).closest('tr');
		var did = $(trObj).attr('data-id');
		$.post('<?php echo U("financial/source");?>', {op_type:'delete', src: 'bus', id:did}, function(data){
			if (data.result != null) {
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
					$(trObj).remove();
	...
	// 搜索用户数据
	constructionPage($('.list-page'), 1, '<?php echo ($page_count); ?>', findPageData, true);	
	function findPageData(p) {			
		...
		if (searchval != '') {
			jsonData['cdsstr'] = 'id|=|'+searchval+'|OR|line|LIKE|%'+searchval+'%|OR|name|LIKE|%'+searchval+'%|OR|bus_no|LIKE|%'+searchval+'%|OR|bus_logo|LIKE|%'+searchval+'%|OR|distance|LIKE|%'+searchval+'%|OR|dynamic_type|LIKE|%'+searchval+'%|OR|contacts|LIKE|%'+searchval+'%|OR|contacts_mob|LIKE|%'+searchval+'%|OR';
		}
				...
				if (data.ds != null && data.ds != 'undefined') {	
					for (var i = 0; i < data.ds.length; i ++){
						...
						var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">';
							vhtml +='	<td>'+d.id+'</td>';
							vhtml +='	<td>'+d.name+'</td>';
							vhtml +='	<td>品牌:'+d.bus_logo+'<br />车牌号:'+d.bus_no+'<br />车距:'+d.distance+'<br />动力类型:'+dynamicType+'<br />车龄:'+d.years+'</td>';
							vhtml +='	<td>'+conds+'</td>';
