...
<?php if(!empty($modal_select_image)): ?><style type="text/css">		
						...
						<div class="col-sm-9 gallery-right">
							...
							<div class="album-header">
								...
								<ul class="album-options list-unstyled list-inline">
									...
									<li>
										<a href="<?php echo U('image/upload');?>">
											<i class="fa-upload"></i>
											上传
										</a>
									</li>
...
<script>
		...
		// 初始化翻页
		constructionPage('.modal-page', 1, changeModalArticleSelectPage, true);
	    ...
	    function changeModalArticleSelectPage(p) {
	    	...
	    	$.post('<?php echo U("line/article");?>', jsonData, function(data){	    		
				if (typeof(data.ds) != 'undefined' && data.ds != null) {
					...
					// 根据情况判断是否重构分页
					constructionPage('.modal-page', data.page_count, changeModalArticleSelectPage);
				} 
	    	});
	    }
...
<script>
	...
	// 初始化翻页
	constructionPage($('#modalLineSelect').find('.modal-page'), 1, changeModalLineSelectPage, true);
...
//    				console.log(d.id + ':' + d.title+',assembly:'+assemblyPoint+',dest:'+destination+',batch_count:'+batchCount+',img:'+img_src);    				
				...
				// 根据情况判断是否重构分页
				constructionPage($('#modalLineSelect').find('.modal-page'), data.page_count, changeModalLineSelectPage);
			} 
    	});
    }
...
//							       +    	'<h5><a href="javascript:;" onclick="addIframe('+ d.title + ',' + d.href +');">' + d.title + '</a></h5>'
					...
					// 根据情况判断是否重构分页
					constructionPage($("#modalVideoSelect").find('.modal-page'), data.page_count, changeModalVideoSelectPage);
				} 
	    	});
	    }
		...
		<div class="sidebar-menu toggle-others collapsed">
			...
			<div class="sidebar-menu-inner">	
				...
				<ul id="main-menu" class="main-menu">
					...
					<?php if(line_system == true): ?><li <?php echo C('MENU_ACTIVE')=='line' ? 'class="active opened"' : '';?>>
						<a href="#">
							<i class="fa-th"></i>
...
//						});
					...
					<?php if(task_alert == true): ?><li <?php echo C('MENU_ACTIVE')=='alert_task' ? 'class="active opened"' : '';?>>
						<a href="#">
		...
		<div class="panel-body">			
				...
				<thead>
					...
					<tr>
						...
						<th>
							...
							<?php if(is_array($TaskProject)): $i = 0; $__LIST__ = $TaskProject;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tp): $mod = ($i % 2 );++$i; echo ($tp["type_desc"]); ?>/<?php endforeach; endif; else: echo "" ;endif; ?>
			...
			<div class="row">
				...
				<div class="col-md-1">
					...
					<a href="<?php echo U('task/task','op_type=create');?>" class="btn btn-primary" style="float: right" id="btn_create_task">新建任务</a>
...
//			console.log(JSON.stringify(e));	
	...
	function arrangementAdmin(taskId, adminId, account, bAdded) {
		...
		$.post('<?php echo U("task/task");?>', jsonData, function(data){
...
//			console.log(JSON.stringify(data));
	...
	function _editFun(aid) {
		...
		$.getJSON('<?php echo U("task/task");?>', jsonData, function(data){
