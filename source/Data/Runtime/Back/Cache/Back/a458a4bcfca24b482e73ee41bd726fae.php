						...
						<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
							<table class="table table-bordered table-striped dataTable no-footer" id="table_data" role="grid" aria-describedby="example-2_info">
								...
								<tbody class="middle-align coupon-tbody">
									<!--<?php echo p_a($teams);?>-->
									<?php if(is_array($teams)): $i = 0; $__LIST__ = $teams;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i; if($key % 2 == 0): ?><tr role="row" class="odd" data-id="<?php echo ($t["id"]); ?>">
										<?php else: ?>
											<tr role="row" class="even" data-id="<?php echo ($t["id"]); ?>"><?php endif; ?>
											<td><?php echo ($t["id"]); ?></td>
											<td><?php echo ($t["team_code"]); ?></td>
											<td><?php echo ($t["line_data"]["title"]); ?><br><?php echo ($t["batch_show_text"]); ?></td>
											<td><?php echo ($t["max_member"]); ?></td>
											<td><?php echo ($t["leader_show_text"]); ?></td>
											<td><?php echo ($t["admin_show_text"]); ?></td>
											<td><?php echo ($t["dijie_data"]["name"]); ?></td>
											<td><?php echo ($t["insurance_data"]["name"]); ?></td>
											<td><?php echo ($t["balance"]); ?></td>
											<td><?php echo ($t["create_time"]); ?></td>
											<td>
												<a href="<?php echo U('financial/team');?>/op/new/id/<?php echo ($t["id"]); ?>" class="btn btn-secondary btn-sm btn-icon icon-left" target="_blank">团期详细</a>
											</td>
...
<script type="text/javascript">	
			...
			if (data.ds != null && data.ds != 'undefined') {	
				for (var i = 0; i < data.ds.length; i ++){
					...
					var vhtml =	'<tr role="row" class="'+trClass+'" data-id="'+d.id+'">'+
							...
							'	<td>'+d.create_time+'</td>'+
							'	<td>'+
							'		<a href="'+'<?php echo U("financial/team");?>/op/new/id/'+d.id+'" class="btn btn-secondary btn-sm btn-icon icon-left" target="_blank">团期详细</a>'+
							'	</td>'+
							'</tr>';
	...
	// 搜索用户数据
	constructionPage($('.list-page'), 1, '<?php echo ($page_count); ?>', findTeam, true);	
