			...
			<div class="panel panel-color panel-info panel-content-pos">
				...
				<div class="panel panel-color panel-content">
                    <table id="test" class="table aspTable lcnp" cellpadding="0" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <?php if($group_assign == true): ?><th width="10%"><input type="checkbox" id="checkAll" /></th>
	                                <th width="10%">类型编号</th>
	                                <th width="20%">类型描述</th>
	                                <th width="20%">类型键值</th>
	                                <th width="20%">
	                                	<?php if(is_array($adminType)): $i = 0; $__LIST__ = $adminType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$at): $mod = ($i % 2 );++$i; echo ($at["type_desc"]); ?>/<?php endforeach; endif; else: echo "" ;endif; ?>
	                                </th>
	                                <?php if(($grant_update == true) OR ($grant_delete == true)): ?><th width="20%" class="TAC">编辑/删除</th><?php endif; ?>
	                           	<?php elseif($typeObj == 'request_data'): ?>
	                           	...
	                           	<?php elseif($typeObj == 'order_state'): ?>
                                ...
                                <?php else: ?>
	                                ...
	                                <th width="30%">类型描述</th>
	                                <th width="30%">类型键值</th>
	                                <?php if(($grant_update == true) OR ($grant_delete == true)): ?><th width="20%" class="TAC">编辑/删除</th><?php endif; endif; ?>
                            </tr>
                        </thead>
                    </table>   
                    
					<div class="row">
						<?php if($group_bind_grant == true): ?><a href="#" class="btn btn-primary" id="btn_assign_grant">分配权限</a><?php endif; ?>						
						<?php if($grant_create == true): ?><a href="#" class="btn btn-primary" id="btn_create_type">创建类型</a><?php endif; ?>
						<?php if($grant_delete == true): ?><a href="#" class="btn btn-danger" id="btn_delete_type">删除类型</a><?php endif; ?>
					</div>
				</div>
			</div>		
		</div>
...
<script type="text/javascript">
	...
	function initModalDataEdit() {
		...
		for (var k=0; k<$(needs).length; k++) {
			if ('<?php echo ($typeObj); ?>' == needs[k].type_obj) {
				modelData = needs[k];
				break;
			}
		}
...
//				console.log(JSON.stringify(data.aaData));
	...
	function appendMiddleCol(nTd, sData, oData, iRow, iCol) {		
		...
		if (typeObj == 'admin_grant_group') {
			if ('<?php echo ($group_bind_admin_type); ?>' == true) {
...
//	    $(nTd).html('<a href="#" class="btn btn-secondary btn-sm btn-icon icon-left" onclick="_editFun('+id+')">编辑</a>');
	...
	*/
	function _assignFun() {
		location.href = '<?php echo U("grant/group");?>'+'/op/all';
	}
	...
	*/
	function _addFunAjax(ds) {
		...
		console.log(JSON.stringify(jsonData));
		$.post('<?php echo ($ajaxReqUrl); ?>',jsonData,
...
//	            alert(backdata['result']['message']);
	...
	function _grantFun(aid) {
		$.getJSON('<?php echo U("grant/group");?>', {op:'jumpGroup', id:aid}, function(data){
...
//					alert(backData['result']['message']);	
	...
	function _changeFun() {
		...
		var obj = $(this);
		$.post('<?php echo U("grant/list");?>', jsonData, function(data){
			console.log(JSON.stringify(data));
			if (data.result.errno != 0) {
				alert(data.result.message);			
