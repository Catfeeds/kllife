			...
			<div class="panel-body row">
				<div id="dingdan_main" class="col-sm-12">
					...
					<div id="article_content" class="row article_content">
						...
						<hr />
						<?php if(is_array($article["content"])): $ct_i = 0; $__LIST__ = $article["content"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ct): $mod = ($ct_i % 2 );++$ct_i;?><div class="row youji" id="article_<?php echo ($ct_i); ?>" data-id="<?php echo ($ct["id"]); ?>" data-index="<?php echo ($ct["index"]); ?>">
								<div class="col-md-11">
									<div class="form-group">
										<?php if(empty($ct['content']) == true): ?><img class="youji_image" src="<?php echo ($ct["image_url"]); ?>" alt="" width="100%"/>
										<?php else: ?>
											<div class="youji_show_text "><?php echo ($ct["content"]); ?></div>
											<div class="youji_ct_text hidden_ctrl">
												<textarea class="form-control youji_text"><?php echo ($ct["content"]); ?></textarea>
											</div><?php endif; ?>
									</div>
								</div>
								<div class="col-md-1">
									<div class="form-group">
										<?php if(empty($ct['content']) == false): ?><button class="btn btn-secondary btn_youji_edit">编辑</button><?php endif; ?>
...
//		}		
		...
		$(document).ready(function(){
			...
			// 是否是轻旅拍文章
			$('#qinglvpai').val('<?php echo ($article["qinglvpai"]); ?>').trigger('change');
