	...
	<div id="content">
		...
		<div class="commodity-menu">
			...
			<div class="row">	
				...
				<?php if($admin['station_id_data']['key'] === 'main'): ?><div class="col-md-2">						
		...
		<div class="hidden_ctrl template_line" data-id="">
			<div class="single-row-content">
				...
				<div class="single-row-describe">
					<div class="single-row-describe-top">
						<?php if(check_grant('line_sort') == true): ?><div class="single-row-qz">
								<div class="input-group">
					...
					<div class="single-row-describe-bottom">
						...
						<?php if(check_grant('line_edit') == true): ?><button class="btn btn-success test btn-line info">编辑</button><?php endif; ?>						
						<?php if($admin[account] === 'admin'): ?><button class="btn btn-success test btn-line distribute">分销</button><?php endif; ?>
						<?php if(check_grant('line_online') == true): ?><button class="btn btn-warning test btn-line online">上线</button><?php endif; ?>
						<?php if(check_grant('line_hot_recommend_leader') == true): ?><button class="btn btn-warning test btn-line leader_recommend">高性价比</button>
							<button class="btn btn-warning test btn-line recommend">极致体验</button>
							<button class="btn btn-warning test btn-line hot">全网爆款</button><?php endif; ?>
					</div>
				</div>
			</div>
		</div>			
		...
		<div class="list-menu">
			<?php if(is_array($lines)): $i = 0; $__LIST__ = $lines;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$line): $mod = ($i % 2 );++$i;?><div class="single-row" data-id="<?php echo ($line["id"]); ?>">
					<div class="single-row-content">
						<div class="single-row-img">
						...
						<div class="single-row-describe">
							<div class="single-row-describe-top">
								<?php if(check_grant('line_sort') == true): ?><div class="single-row-qz">
										<div class="input-group">
											<input class="sort" type="text" maxlength="4" value="<?php echo ($line["sort"]); ?>" class="form-control no-right-border form-focus-purple">
											<div class="input-group-addon">
												<i>权重</i>
											</div>
										</div>									
								...
								<a type="button" class="btn input-group-addon copy-line">产品复制</a>
								<a type="button" class="btn input-group-addon export-batch">导出批次</a>
								<h4 class="title"><?php echo ($line["title"]); ?>(id:<?php echo ($line["id"]); ?>)</h4>
								...
								<p>
									...
									<u class="click_count">点击量：<?php echo ($line['click_count']); ?>次</u>
								</p>
								<p>
									<u class="destination">目的地：<?php echo ($line["destination_show_text"]); ?></u>
									<u class="station">站点：<?php echo ($line['station_show']); ?></u>
								</p>
								<div class="single-row-describe-price">
									<h6>
										<s class="price"><?php echo ($line["start_price_adult"]); ?>元</s>
										<strong class="cut_price"><?php echo ($line["start_price_adult"]); ?></strong>元起
									</h6>
								</div>
							...
							<div class="single-row-describe-bottom">
								<button class="btn btn-success test btn-line preview">预览</button>
								<?php if(check_grant('line_edit') == true): if($line["lock"] == 1): ?><button class="btn btn-success test btn-line info">已锁定</button>
									<?php else: ?>
										<button class="btn btn-success test btn-line info">编辑</button><?php endif; endif; ?>
								<?php if($line["extra"] == 0 and $admin["account"] == 'admin'): if($line["distribute"] == 1): ?><button class="btn btn-warning test btn-line distribute">分销</button>
									<?php else: ?>
										<button class="btn btn-gray test btn-line distribute">分销</button><?php endif; endif; ?>								
								<?php if(check_grant('line_online') == true): if($line["online"] == 1): ?><button class="btn btn-warning test btn-line online">下架</button>
									<?php else: ?>
										<button class="btn btn-gray test btn-line online">上架</button><?php endif; endif; ?>
								<?php if(check_grant('line_hot_recommend_leader') == true): if($line["leader_recommend"] == 1): ?><button class="btn btn-warning test btn-line leader_recommend">高性价比</button>
									<?php else: ?>
										<button class="btn btn-gray test btn-line leader_recommend">高性价比</button><?php endif; ?>
									<?php if($line["recommend"] == 1): ?><button class="btn btn-warning test btn-line recommend">极致体验</button>
									<?php else: ?>
										<button class="btn btn-gray test btn-line recommend">极致体验</button><?php endif; ?>
									<?php if($line["hot"] == 1): ?><button class="btn btn-warning test btn-line hot">全网爆款</button>
									<?php else: ?>
										<button class="btn btn-gray test btn-line hot">全网爆款</button><?php endif; endif; ?>
							</div>
						</div>
					</div>
	...
	<script type="text/javascript">
		...
		function setLine() {
			...
			if ($(btnObj).hasClass('online')) {
				...
				var w = window.open();
				$.post('<?php echo U("line/create");?>',{op_type:'preview', id:line_id},function(data){
					if (data.result.errno != 0) {
						toastr.error(data.result.message);
					}
					if (data.salt != null) {
		...
		$('.copy-line').click(function(){
			...
			showLoading(true);
			$.post('<?php echo U("line/copy");?>', jsonData, function(data){
				showLoading(false);
				if (data.result.errno == 0) {
					toastr.success(data.result.message);
				} else {
		...
		$('.export-batch').click(function(){
			var lineId =  $(this).closest('.single-row').attr('data-id');
			location.href = '<?php echo U("line/export");?>/type/batch/id/'+lineId;
		});
