...
<div class="page">
	<div class="content">
		...
		<div class="module01">
			<div class="swiper-container" >
				<div class="swiper-wrapper">
					<?php $__FOR_START_922538007__=1;$__FOR_END_922538007__=7;for($i=$__FOR_START_922538007__;$i <= $__FOR_END_922538007__;$i+=1){ $img = $set['lunbo'.$i]; $url = $set['lunbo'.$i.'_url']; ?>
		...
		<!--<div class="special">
			...
			<?php $__FOR_START_1826832981__=0;$__FOR_END_1826832981__=4;for($i=$__FOR_START_1826832981__;$i < $__FOR_END_1826832981__;$i+=1){ $rowClass = intval($i / 2) == 0 ? 'border-bottom: 1px solid #dcdcdc;' : ''; $zt = $set['zhuanti'.$i]; ?>
		...
		<div class="hot-sale">
			<div class="recommend-result">
				<?php $__FOR_START_36649781__=0;$__FOR_END_36649781__=10;for($i=$__FOR_START_36649781__;$i < $__FOR_END_36649781__;$i+=1){ $hotLine = $set['hot_line_recommend'.$i]; ?>
					<?php if(empty($hotLine) == false): ?><div class="item">
							...
							<div class="price">
								<?php if (empty($hotLine['check_price'])) { echo('<span class="right"><span class="money">核算中</span></span>'); }else { $price = explode('.', $hotLine['start_price_adult']); echo('<span class="right">￥<span class="money">'.$price[0].'</span>元起</span>'); } ?>
							</div>
			...
			<div class="bournTopList" id="bournFixed">
				<div class="scrollBox" id="scrollBox">
					<ul class="bournList displayBox" id="j-bournTitle">
						<li data-id="0" class="open-preloader">荐</li>
						<?php $themeMap = array('pc_home_menu_xtmlx'=>true, 'pc_home_menu_xinlvpai'=>true, 'pc_home_menu_sheying'=>true, 'pc_home_menu_zhuti'=>true); ?>						
						<?php if(is_array($menus)): $mk = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tm): $mod = ($mk % 2 );++$mk; if(empty($themeMap[$tm['item_key']]) == false): if($tm['item_key'] == 'pc_home_menu_zhuti'): ?><li data-id="<?php echo ($tm["id"]); ?>" class="open-preloader" id="shenduyou">深度游</li>
								<?php else: ?>
				...
				<div class="bournChildList fn-clear" id="j-bournTitleChild">
					<?php if(is_array($left_menus)): $i = 0; $__LIST__ = $left_menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lm): $mod = ($i % 2 );++$i; if($lm['item_key'] == 'pc_home_left_menu_hot'): ?><ul style="display: none;" data-key="<?php echo ($lm['item_key']); ?>">
								<li class="hover" data-id="0">推荐</li>
								<?php if(is_array($lm["child"])): $i = 0; $__LIST__ = $lm["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lmc): $mod = ($i % 2 );++$i;?><li data-id="<?php echo ($lmc["id"]); ?>"><?php echo ($lmc["item_desc"]); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
					...
					<?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tm): $mod = ($i % 2 );++$i; if(empty($themeMap[$tm['item_key']]) == false): ?><ul style="display: none;" data-key="<?php echo ($tm['item_key']); ?>">
								<li class="hover open-preloader" data-id="0">推荐</li>
		...
		<div class="destination" style="position: relative;">
			<div class="recommend-result list_container new" style="position: absolute; top: 0; left: 0; width: 100%;margin-bottom:80px;" data-index="0" data-more="1">
			</div>
			<div class="recommend-result list_container old" style="position: absolute; top: 0; left: 100%; width: 100%;margin-bottom:80px;" data-index="0" data-more="1">
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	$('#j-bournTitle').find('li:first').addClass('active');
    ...
    // 目的地框固定
	var jheight = $("#j-bournBox").height() + 110;
	...
	function fixedDestination() {
        ...
        if( startPos <= p ){
            ...
            $('#j-bournBox').css({'position':'fixed',
                ...
                'width':'100%',
                'z-index':'999'});
            $(".destination").css("margin-top",jheight-110)
        }else{
            //清除固定
...
//  	}
//  	lastBottom = b;
    	var destB = $('.destination').offset().top + $('.destination').height();
    	console.log('db:'+destB+',b:'+b);
    	if (destB - b < 100) {
    		loadMoreData();
    	}
    }
    ...
    function changeDataContainer() {
    	var newObj = $('.list_container.new');
    	var oldObj = $('.list_container.old');
    	$(newObj).removeClass('new').addClass('old');
    	$(oldObj).removeClass('old').addClass('new');
    	$(oldObj).animate({left:'0px',top:'0px'}, 500);
    	$(newObj).animate({left:'100%',top:'0px'}, 500);
...
//    	$('.list_container').children().remove();
	...
	// 查找产品		
	var pageLoad = new PageLoading();
	function loadMoreData(switchTab) {
		var tabClass = switchTab != null ? 'old' : 'new';		
		var containerObj = $('.list_container.'+tabClass);
		var noMoreData = $(containerObj).attr('data-more');
		if (noMoreData == '0' || pageLoad.isLoading() == true) {
			return false;
		}
...
//		console.log(JSON.stringify(jsonData));
		pageLoad.loading(true, false);
		$.post('<?php echo U("line/list");?>', jsonData, function(data){
			pageLoad.loading(false, false);
			if (data.result.errno == 0) {
				if (data.ds != null && data.ds != 'undefined') {
					for (var i=0; i < data.ds.length; i++) {
						...
						if (d.check_price == 0) {
							$(itemObj).find('.price .right').html('核算中');
						} else {
							var price = d.start_price_adult.split('.')
							$(itemObj).find('.price .money').html(price[0]);
						}
