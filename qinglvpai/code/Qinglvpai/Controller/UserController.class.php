...
class UserController extends QlpBaseController {	
	...
	public function registerAction() {
		if (IS_POST) {			
			...
			if (array_key_exists('phone', $ds)) {
				$code = I('post.code', 0);
				$result = MyHelp::checkSMS('qinglvpai_register', $ds['phone'], $code);
	...
	public function smsLoginAction() {
		...
		if (IS_POST) {
			...
			$data['result'] = MyHelp::checkSMS('qinglvpai_sms_login', $phone, $code);
...
//				$data['jumpUrl'] = U('index/welcome');
	...
	public function forgotPasswordAction() {
		if (IS_POST) {
			...
			$result = MyHelp::checkSMS('qinglvpai_forgot_password', $phone, $code);
	...
	private function modifyPhone($aa) {
		...
		if (is_error($user) === false) {	
			...
			if ($aa['check'] == 0) {
				...
				$data['result'] = MyHelp::checkSMS('qinglvpai_modify_phone', $aa['phone'], $aa['code']);
				if (error_ok($data['result']) == true) {
					$data['check_result'] = $data['result'];
