...
abstract class QlpBaseController extends Controller {
	...
	protected function _initTemplateInfo() {
		...
		// 底部问题链接
		$questType = S('qlp_home_bottom_question');
		if (empty($questType) || MyHelp::checkConfigOverdue('qlp_home_bottom_question', $questType['config_update_time']) === true) {
			$questClass = array(
				'question_qinglvpai_type_regist_login'=>'',
				'question_qinglvpai_type_activity'=>'',
				'question_qinglvpai_type_pay'=>'',
				'question_qinglvpai_type_offen'=>'clear-footer-content-marginLeft',
			);
			...
			$questType['config_update_time'] = MyHelp::getConfigUpdateTime('qlp_home_bottom_question');
			S('qlp_home_bottom_question', $menuItems, 0);
		}
