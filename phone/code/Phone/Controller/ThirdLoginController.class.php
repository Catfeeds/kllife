...
class ThirdLoginController extends PhoneBaseController {
	...
	//微信手机端登录 - 微信公众平台获取用户信息
	public function OpenWXLoginAction(){
		$type	= I('get.type', false);
		$openid	= I('get.openid', false);
		
		if(!empty($type) and !empty($openid)){
			$this->calllogin($type, $openid);
		}else{
			$data['result'] = error(-1, '请求参数不完整');
			return $this->ajaxReturn($data);
		}		
